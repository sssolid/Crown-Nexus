...
from app.models.product import (
    Product, ProductMeasurement, ProductPricing, ProductStock,
    Manufacturer, PriceType
)
from app.models.currency import Currency
from app.models.reference import Warehouse
from app.schemas.product import (
    ProductCreate, ProductMeasurementCreate,
    ...
    async def get_existing_entities(
        ...
        try:
            query = select(model).where(
                getattr(model, id_field).in_(id_values),
            ...
            logger.error(
                f"Error fetching existing {model.__name__} entities: {str(e)}"
            )
...
class ProductMeasurementImporter(AS400BaseImporter[ProductMeasurementCreate]):
    ...
    async def import_data(
        self, data: List[ProductMeasurementCreate]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            existing_products = await self.get_existing_entities(
                id_field="id",
                id_values=product_ids,
                model=Product
            )

            # Get manufacturer IDs
            manufacturer_ids = [
                m.manufacturer_id for m in data
            ...
            for measurement_data in data:
                try:
                    ...
                    if (
                        measurement_data.manufacturer_id is not None and
                        measurement_data.manufacturer_id not in existing_manufacturers
                    ):
                        stats["errors"] += 1
                        stats["error_details"].append({
                            "product_id": str(measurement_data.product_id),
                            "manufacturer_id": str(measurement_data.manufacturer_id),
                    ...
                    query = select(ProductMeasurement).where(
                        ...
                        ProductMeasurement.manufacturer_id == measurement_data.manufacturer_id,
...
class ProductStockImporter(AS400BaseImporter[ProductStockSchema]):
    ...
    async def import_data(
        self, data: List[ProductStockSchema]
    ) -> Dict[str, Any]:
        """
