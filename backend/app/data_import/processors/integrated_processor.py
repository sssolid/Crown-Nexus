...
class IntegratedProductProcessor(GenericProcessor[ProductCreate]):
    """
    Integrated processor for product data that handles descriptions, marketing,
    and pricing in a unified manner.
    """
    ...
    def __init__(self, source_type: str) -> None:
        ...
        Args:
            source_type: Type of data source (filemaker, as400, csv)
        """
        super().__init__(
    ...
    def _create_field_alias_map(self, complex_field: str) -> Dict[str, str]:
        """
        Create a mapping between external field names and internal field types.
    ...
    def _process_complex_field(
        ...
        Args:
            processed_record: The record being processed
        ...
        if complex_field == "marketing":
            bullet_pattern = re.compile(r"bullet(\d+)", re.IGNORECASE)
            bullet_fields = []

            # First collect all bullet fields
            ...
            # Sort by position and add to items
    ...
    async def process_pricing(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Process pricing data for products.
    ...
    async def validate_pricing(self, data: List[Dict[str, Any]]) -> List[ProductPricingImport]:
        ...
        Args:
            data: Processed pricing data

        Returns:
            List of validated ProductPricingImport objects
        """
        validated_data = []
        validation_errors = []
