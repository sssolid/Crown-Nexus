...
def import_autocare(
    database: AutoCareDatabase = typer.Argument(
        AutoCareDatabase.ALL,
        help="AutoCare database to import (vcdb, pcdb, padb, qdb, or all)",
    ),
    source_dir: Path = typer.Option(
        ...,
        "--source-dir",
        "-s",
        help="Directory containing the AutoCare data files",
        exists=True,
        file_okay=False,
        dir_okay=True,
    ),
    format: Optional[SourceFormat] = typer.Option(
        None,
        "--format",
        "-f",
        help="Format of the source data (pipe, json, etc.). Auto-detected if not specified.",
    ),
    ...
    dry_run: bool = typer.Option(
        ...
        "--dry-run",
        "-d",
        help="Validate the source data but don't import it",
    ),
    track_in_history: bool = typer.Option(
        True,
        "--track-history/--no-track-history",
        help="Track the import in the sync history",
    ),
    ...
    This command imports VCdb, PCdb, PAdb, and Qdb data from
    various data formats into the application database.

    Example:
        import-autocare import vcdb --source-dir=/path/to/vcdb_files --format=json
    """
    # Print immediate feedback that command is running
    print("\n" + "=" * 80)
    ...
    try:
        ...
        result = asyncio.run(
            run_import(
                ...
                dry_run=dry_run,
                track_in_history=track_in_history,
                verbose=verbose,
            )
        )
...
async def run_import(
    ...
    Args:
        database: AutoCare database to import
        source_dir: Directory containing the data files
        source_format: Format of the source data
        batch_size: Number of records to process in each batch
        dry_run: Validate the source data but don't import it
        track_in_history: Track the import in the sync history
        verbose: Enable verbose output

    Returns:
        Union[Dict, Dict[str, Dict]]: Import results
    """
    ...
    if database == AutoCareDatabase.ALL:
        databases_to_import = [
            AutoCareDatabase.VCDB,
            AutoCareDatabase.PCDB,
...
async def import_database(
    ...
    if track_in_history:
        try:
            ...
            sync_record = await sync_repo.create_sync(
                entity_type=SyncEntityType.AUTOCARE,
    ...
    try:
        ...
        if not is_valid:
            ...
            return {
                "success": False,
                "message": error_msg,
                "errors": ["Source validation failed. See logs for details."],
            }
        ...
        if dry_run:
            ...
            return {
                "success": True,
                "message": f"Dry run: {database.value} source validation passed",
                "dry_run": True,
            }
...
def get_database_source_dir(source_dir: Path, database: AutoCareDatabase) -> Path:
    ...
    Returns:
        Path: Database-specific source directory
    """
    # Check if the source_dir is the specific database directory
    # Look for version files in either JSON or pipe format
    if (source_dir / "Version.txt").exists() or (source_dir / "Version.json").exists():
        # This appears to be a database-specific directory already
        return source_dir

    # Check for database-specific subdirectories
    patterns = {
        AutoCareDatabase.VCDB: ["*VCdb*", "*VCDB*", "vcdb"],
        AutoCareDatabase.PCDB: ["*PCdb*", "*PCDB*", "pcdb"],
        AutoCareDatabase.PADB: ["*PAdb*", "*PADB*", "padb"],
        AutoCareDatabase.QDB: ["*Qdb*", "*QDB*", "qdb"],
    }

    # Try to find the appropriate subdirectory
