# app/domains/autocare/importers/padb_importer.py
...
class PAdbImporter(FlexibleImporter):
    ...
    def _register_mappings(self) -> None:
        ...
        self.register_table_mapping(
            source_name=f"PartAttributes{file_ext}",
            model_class=PartAttribute,
            field_mapping={
                "pa_id": "PAID",
                "pa_name": "PAName",
                "pa_descr": "PADescr",
            ...
            primary_key="pa_id",
            transformers={
                "pa_id": lambda x: int(x) if x else None,
            },
        )

        # MetaData
        self.register_table_mapping(
            source_name=f"MetaData{file_ext}",
            ...
            primary_key="meta_id",
            transformers={
                "meta_id": lambda x: int(x) if x else None,
                "min_length": lambda x: int(x) if x else None,
                "max_length": lambda x: int(x) if x else None,
            },
        )

        # MeasurementGroup
        self.register_table_mapping(
            source_name=f"MeasurementGroup{file_ext}",
            model_class=MeasurementGroup,
            field_mapping={
                "measurement_group_id": "MeasurementGroupID",
                "measurement_group_name": "MeasurementGroupName",
            },
            primary_key="measurement_group_id",
            transformers={
                "measurement_group_id": lambda x: int(x) if x else None,
            },
        )

        # MetaUOMCode
        self.register_table_mapping(
            source_name=f"MetaUOMCodes{file_ext}",
            model_class=MetaUOMCode,
            field_mapping={
                "meta_uom_id": "MetaUOMID",
                "uom_code": "UOMCode",
                "uom_description": "UOMDescription",
                "uom_label": "UOMLabel",
                "measurement_group_id": "MeasurementGroupId",
            },
            primary_key="meta_uom_id",
            transformers={
                "meta_uom_id": lambda x: int(x) if x else None,
                "measurement_group_id": lambda x: int(x) if x else None,
            },
        )
        ...
        # PartAttributeAssignment
        self.register_table_mapping(
            source_name=f"PartAttributeAssignment{file_ext}",
            model_class=PartAttributeAssignment,
            field_mapping={
                "papt_id": "PAPTID",
            ...
            primary_key="papt_id",
            transformers={
                "papt_id": lambda x: int(x) if x else None,
        ...
        # MetaUomCodeAssignment
        self.register_table_mapping(
            source_name=f"MetaUOMCodeAssignment{file_ext}",
            model_class=MetaUomCodeAssignment,
            field_mapping={
                "meta_uom_code_assignment_id": "MetaUomCodeAssignmentID",
                "papt_id": "PAPTID",
                "meta_uom_id": "MetaUomID",
            },
            primary_key="meta_uom_code_assignment_id",
            transformers={
                "meta_uom_code_assignment_id": lambda x: int(x) if x else None,
                "papt_id": lambda x: int(x) if x else None,
                "meta_uom_id": lambda x: int(x) if x else None,
            },
        )
        ...
        # ValidValue
        self.register_table_mapping(
            source_name=f"ValidValues{file_ext}",
            model_class=ValidValue,
            field_mapping={
                "valid_value_id": "ValidValueID",
                "valid_value": "ValidValue",
            },
            primary_key="valid_value_id",
            transformers={
                "valid_value_id": lambda x: int(x) if x else None,
            },
        )

        # ValidValueAssignment
        self.register_table_mapping(
            source_name=f"ValidValueAssignment{file_ext}",
            model_class=ValidValueAssignment,
            field_mapping={
                "valid_value_assignment_id": "ValidValueAssignmentID",
                "papt_id": "PAPTID",
                "valid_value_id": "ValidValueID",
            },
            primary_key="valid_value_assignment_id",
            transformers={
                "valid_value_assignment_id": lambda x: int(x) if x else None,
                "papt_id": lambda x: int(x) if x else None,
                "valid_value_id": lambda x: int(x) if x else None,
            },
        )
        ...
        self.register_table_mapping(
            ...
            transformers={
                "style_id": lambda x: int(x) if x else None,
            },
        )

        # PartAttributeStyle
        self.register_table_mapping(
            source_name=f"PartAttributeStyle{file_ext}",
            model_class=PartAttributeStyle,
            field_mapping={
                "style_id": "StyleID",
                "papt_id": "PAPTID",
            },
            primary_key="id",  # Using the UUID primary key since this table doesn't have a natural key
            transformers={
                "style_id": lambda x: int(x) if x else None,
                "papt_id": lambda x: int(x) if x else None,
            },
        ...
        # PartTypeStyle
        self.register_table_mapping(
            source_name=f"PartTypeStyle{file_ext}",
            model_class=PartTypeStyle,
            field_mapping={
                "style_id": "StyleID",
                "part_terminology_id": "PartTerminologyID",
            },
