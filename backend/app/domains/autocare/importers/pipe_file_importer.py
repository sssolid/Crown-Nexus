# app/domains/autocare/importers/pipe_file_importer.py
from __future__ import annotations

"""
Pipe delimited file importer for AutoCare data.

This module provides a generic implementation for importing pipe-separated data files
...
logger = get_logger("app.domains.autocare.importers.pipe_file_importer")
...
class PipeFileImporter(Generic[T]):
    ...
    async def validate_source(self) -> bool:
        ...
        if not version_file.exists():
            error_msg = "Version.txt file not found"
            print(f"\nERROR: {error_msg}")
            logger.error(error_msg)
            return False
        else:
            try:
                ...
                print(f"\nWARNING: Could not read Version.txt: {str(e)}")

        # Print mapped files status if verbose
