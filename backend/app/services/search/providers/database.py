    ...
    async def initialize(self) -> None:
        ...
        if self.model_class == Product:
            ...
            for column_name in getattr(self.model_class, "__table__").columns.keys():
                if any(
                    field in column_name.lower()
                    for field in ["name", "title", "description", "id", "code"]
                ):
    ...
    async def search(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if search_term:
                ...
                for field_name in self.searchable_fields:
                    if hasattr(self.model_class, field_name):
                        ...
                        if hasattr(column, "type") and hasattr(
                    ...
                    if hasattr(self.model_class, key):
                        if isinstance(value, dict) and key == "attributes":
                            ...
                            for attr_key, attr_value in value.items():
                                query = query.where(
                                    getattr(self.model_class, key).contains(
                                        {attr_key: attr_value}
                                    )
                                )
                        else:
            ...
            if hasattr(self.model_class, "is_active") and not (
                filters and "is_active" in filters
            ):
                query = query.where(getattr(self.model_class, "is_active") == True)

            self.logger.debug(
                "Database search query built", search_term=search_term, filters=filters
            )
