...
class CustomVisitor(ast.NodeVisitor):
    ...
    def get_source_segment(self, node: ast.AST) -> Optional[str]:
        ...
        if start < 0 or start >= len(self.source_lines) or end >= len(self.source_lines):
            return None
    ...
    def _process_function(self, node: Union[ast.FunctionDef, ast.AsyncFunctionDef], is_async: bool) -> None:
        ...
        for i, arg in enumerate(node.args.kwonlyargs):
            ...
            if default_index >= 0 and default_index < len(node.args.defaults):
                args.append(f"{arg.arg}={self._get_name_from_node(node.args.defaults[default_index])}")
            else:
                args.append(arg.arg)
        ...
        if node.args.vararg:
            ...
            if node.args.vararg.annotation:
                arg_annotations[node.args.vararg.arg] = self._get_name_from_node(node.args.vararg.annotation)

        if node.args.kwarg:
            args.append(f"**{node.args.kwarg.arg}")
            if node.args.kwarg.annotation:
                arg_annotations[node.args.kwarg.arg] = self._get_name_from_node(node.args.kwarg.annotation)
        ...
        is_property = any(d.id == "property" if isinstance(d, ast.Name) else False for d in node.decorator_list)
            ...
            for target in node.targets:
                if isinstance(target, ast.Name):
                    if isinstance(node.value, ast.Constant):
                        ...
                        self.global_vars[target.id] = self.get_source_segment(node.value) or "..."
        self.generic_visit(node)
        ...
        if isinstance(node, ast.Name):
            ...
            for i, key in enumerate(node.keys):
                if key is None:  # Handle **unpacked
                    ...
                    key_values.append(f"{self._get_name_from_node(key)}: {self._get_name_from_node(node.values[i])}")
            return f"{{{', '.join(key_values)}}}"
        elif isinstance(node, ast.Call):
            args = [self._get_name_from_node(arg) for arg in node.args]
            keywords = [f"{kw.arg}={self._get_name_from_node(kw.value)}" for kw in node.keywords]
...
class CodeStructureMapper:
    ...
    def _process_package(self, package_path: Path) -> PackageInfo:
        ...
        for item in os.scandir(package_path):
            ...
            elif item.is_file() and item_path.suffix == ".py" and item_path.name != "__init__.py":
    ...
    def _filter_private_members(self, module_info: ModuleInfo) -> None:
        ...
        module_info.functions = {
            name: info for name, info in module_info.functions.items()
        ...
        for class_name, class_info in module_info.classes.items():
            if class_name.startswith("_") and not (class_name.startswith("__") and class_name.endswith("__")):
                continue

            # Filter private methods within classes
            class_info.methods = {
                name: info for name, info in class_info.methods.items()
                if not name.startswith("_") or name.startswith("__") and name.endswith("__")
            }

            # Filter private attributes
            class_info.class_attributes = {
                name: value for name, value in class_info.class_attributes.items()
                if not name.startswith("_") or name.startswith("__") and name.endswith("__")
            }
        ...
        module_info.global_variables = {
            name: value for name, value in module_info.global_variables.items()
            if not name.startswith("_") or name.startswith("__") and name.endswith("__")
        }
    ...
    def export_markdown(self, output_path: Optional[Path] = None) -> Optional[str]:
        ...
        if self.project_info.packages:
            ...
            for package_name, package_info in sorted(self.project_info.packages.items()):
    ...
    def _project_info_to_dict(self) -> Dict[str, Any]:
        ...
        for module_name, module_info in self.project_info.modules.items():
            project_dict["modules"][module_name] = self._module_info_to_dict(module_info)

        # Convert packages
        for package_name, package_info in self.project_info.packages.items():
            project_dict["packages"][package_name] = self._package_info_to_dict(package_info)
    ...
    def _package_info_to_dict(self, package_info: PackageInfo) -> Dict[str, Any]:
        ...
        if package_info.init_module:
            package_dict["init_module"] = self._module_info_to_dict(package_info.init_module)

        # Convert modules
        for module_name, module_info in package_info.modules.items():
            package_dict["modules"][module_name] = self._module_info_to_dict(module_info)

        # Convert subpackages
        for subpackage_name, subpackage_info in package_info.subpackages.items():
            package_dict["subpackages"][subpackage_name] = self._package_info_to_dict(subpackage_info)
    ...
    def _module_to_markdown(self, module_info: ModuleInfo, header_level: int) -> List[str]:
        """
        ...
        if module_info.functions:
            ...
            for func_name, func_info in sorted(module_info.functions.items()):
                async_prefix = "async " if func_info.is_async else ""
                decorators = "\n".join([f"@{decorator}" for decorator in func_info.decorators])
                if decorators:
                    decorators += "\n"
        ...
        if module_info.classes:
            ...
            for class_name, class_info in sorted(module_info.classes.items()):
                ...
                decorators = "\n".join([f"@{decorator}" for decorator in class_info.decorators])
                if decorators:
                    decorators += "\n"
                ...
                if class_info.methods:
                    ...
                    for method_name, method_info in sorted(class_info.methods.items()):
                        ...
                        lines.append("```python")
                        lines.append(f"{property_prefix}{other_decorators}    {signature}")
                        if method_info.docstring and self.include_docstrings:
                            lines.append(f"        \"\"\"{method_info.docstring}\"\"\"")
                        lines.append("```")
    ...
    def _package_to_markdown(self, package_info: PackageInfo, header_level: int) -> List[str]:
        """
        ...
        if package_info.init_module:
            ...
            init_lines = self._module_to_markdown(package_info.init_module, header_level + 1)
        ...
        if package_info.subpackages:
            for subpackage_name, subpackage_info in sorted(package_info.subpackages.items()):
                lines.extend(self._package_to_markdown(subpackage_info, header_level + 1))
    ...
    def _module_to_text(self, module_info: ModuleInfo, indent_level: int) -> List[str]:
        ...
        """
        indent = "  " * indent_level
        lines = [f"{indent}{module_info.name} ({module_info.path.relative_to(self.project_info.root_path)})"]
        ...
        if module_info.classes:
            ...
            for class_name, class_info in sorted(module_info.classes.items()):
                ...
                if class_info.methods:
                    ...
                    for method_name, method_info in sorted(class_info.methods.items()):
                        async_prefix = "async " if method_info.is_async else ""
                        property_prefix = "@property " if method_info.is_property else ""
    ...
    def _package_to_text(self, package_info: PackageInfo, indent_level: int) -> List[str]:
        """
        ...
        """
        indent = "  " * indent_level
        lines = [f"{indent}{package_info.name}/ ({package_info.path.relative_to(self.project_info.root_path)})"]

        # Init module
        if package_info.init_module:
            lines.append(f"{indent}  __init__.py:")
            init_lines = self._module_to_text(package_info.init_module, indent_level + 2)
        ...
        if package_info.subpackages:
            ...
            for subpackage_name, subpackage_info in sorted(package_info.subpackages.items()):
                subpackage_lines = self._package_to_text(subpackage_info, indent_level + 2)
    ...
    def _generate_mermaid_classes(self) -> Tuple[List[str], List[str]]:
        ...
        def process_module(module_info: ModuleInfo, package_path: str = "") -> None:
            ...
            for class_name, class_info in module_info.classes.items():
                ...
                for method_name, method_info in class_info.methods.items():
                    ...
                    return_str = f" : {method_info.return_annotation}" if method_info.return_annotation else ""
                ...
                for base_class in class_info.bases:
                    if "." not in base_class:
                        ...
                        if base_class in module_info.classes:
                            base_full_name = f"{module_prefix}{module_info.name}.{base_class}"
                            relationships.append(f"  {base_full_name} <|-- {full_class_name}")
                    else:
        ...
        def process_package(package_info: PackageInfo, package_path: str = "") -> None:
            current_path = f"{package_path}.{package_info.name}" if package_path else package_info.name
