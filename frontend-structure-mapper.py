...
class FrontendProjectInfo:
    ...
    structure: DirectoryInfo = field(default_factory=lambda: DirectoryInfo(name="", path=Path(".")))
...
class VueParser:
    ...
    def parse(self, file_path: Path) -> VueComponentInfo:
        ...
        try:
            ...
            if script_match:
                ...
                script_lang = "ts" if '<script lang="ts"' in content or '<script setup lang="ts"' in content else "js"
                is_setup = '<script setup' in content
                
                if is_setup:
                    component_info.component_type = "Composition API (script setup)"
                    self._parse_script_setup(script_content, component_info, script_lang)
                else:
                    ...
                    if "defineComponent" in script_content and (
                        ...
                        self._parse_composition_api(script_content, component_info, script_lang)
                    else:
                        component_info.component_type = "Options API"
                        self._parse_options_api(script_content, component_info, script_lang)
    ...
    def _parse_script_setup(self, script_content: str, component_info: VueComponentInfo, lang: str) -> None:
        """
        ...
        # Extract defineProps
        props_match = re.search(r'const\s+props\s*=\s*defineProps<([^>]+)>\(\)', script_content)
        if props_match:
        ...
        # Extract defineEmits
        emits_match = re.search(r'const\s+emit\s*=\s*defineEmits<([^>]+)>\(\)', script_content)
        if emits_match:
            ...
            emits_match = re.search(r'const\s+emit\s*=\s*defineEmits\(\s*(\[[^\]]+\])\s*\)', script_content)
            if emits_match:
                # Array style emits
        ...
        reactive_matches = re.findall(r'const\s+(\w+)\s*=\s*reactive\(([^)]+)\)', script_content)
        for match in reactive_matches:
            name, value = match
            component_info.reactive_state[name] = value.strip()
        
        # Extract computed properties
        computed_matches = re.findall(r'const\s+(\w+)\s*=\s*computed\(\s*\(\)\s*=>\s*([^}]+)\)', script_content)
        for match in computed_matches:
            name, expression = match
            computed_info = ComputedInfo(
                name=name,
                getter=expression.strip()
            )
            component_info.computed[name] = computed_info
        
        # Extract functions/methods
        function_matches = re.findall(r'(async\s+)?function\s+(\w+)\s*\(([^)]*)\)[^{]*{', script_content)
        ...
        arrow_matches = re.findall(r'const\s+(\w+)\s*=\s*(async\s+)?\([^)]*\)\s*=>', script_content)
        ...
        # Extract watchers
        watch_matches = re.findall(r'watch\(\s*(?:\/\*[^*]*\*\/\s*)?([^,]+),\s*(?:\/\*[^*]*\*\/\s*)?\((?:[^)]+)\)\s*=>\s*{', script_content)
        for match in watch_matches:
            target = match.strip()
            watcher_info = WatchInfo(
                target=target,
                handler=f"[Inline watcher for {target}]"
            )
            component_info.watchers[target] = watcher_info
        
        # Extract provide/inject
        provide_matches = re.findall(r'provide\(\s*[\'"]?(\w+)[\'"]?\s*,', script_content)
        for match in provide_matches:
            component_info.provide.append(match)
        
        inject_matches = re.findall(r'const\s+(\w+)\s*=\s*inject\(\s*[\'"]?(\w+)[\'"]?', script_content)
    ...
    def _parse_composition_api(self, script_content: str, component_info: VueComponentInfo, lang: str) -> None:
        """
        ...
        setup_match = re.search(r'setup\s*\(([^)]*)\)\s*{([\s\S]*?)return\s+({[\s\S]*?});?\s*}', script_content)
        if setup_match:
            ...
            reactive_matches = re.findall(r'const\s+(\w+)\s*=\s*reactive\(([^)]+)\)', setup_body)
            for match in reactive_matches:
                name, value = match
                component_info.reactive_state[name] = value.strip()
            
            # Process computed properties
            computed_matches = re.findall(r'const\s+(\w+)\s*=\s*computed\(\s*\(\)\s*=>\s*([^}]+)\)', setup_body)
            for match in computed_matches:
                name, expression = match
                computed_info = ComputedInfo(
                    name=name,
            ...
            # Process methods
            function_matches = re.findall(r'const\s+(\w+)\s*=\s*(async\s+)?\([^)]*\)\s*=>\s*{', setup_body)
            for match in function_matches:
                name, is_async = match
                if name not in component_info.computed and name not in component_info.refs:
                    method_info = MethodInfo(
    ...
    def _parse_options_api(self, script_content: str, component_info: VueComponentInfo, lang: str) -> None:
        """
        ...
        for hook in hooks:
            hook_match = re.search(rf'{hook}\s*\(\s*\)\s*{{([\s\S]*?)}}', script_content)
            if hook_match:
                hook_content = hook_match.group(1).strip()
                hook_info = HookInfo(
                    name=hook,
                    source_code=hook_content
                )
                component_info.hooks[hook] = hook_info
        
        # Extract provide/inject
        provide_match = re.search(r'provide:\s*({[\s\S]*?})(?:,\s*\w+:|,\s*}|$)', script_content)
        if provide_match:
        ...
        if inject_match:
            ...
            inject_match = re.search(r'inject:\s*({[\s\S]*?})(?:,\s*\w+:|,\s*}|$)', script_content)
            if inject_match:
        ...
        # Extract directives
        directives_match = re.search(r'directives:\s*({[\s\S]*?})(?:,\s*\w+:|,\s*}|$)', script_content)
        if directives_match:
    ...
    def _parse_props_interface(self, props_interface: str, component_info: VueComponentInfo) -> None:
        """
    ...
    def _parse_props_object(self, props_obj: str, component_info: VueComponentInfo) -> None:
        """
        ...
        prop_entries = re.findall(r'[\'"]?(\w+)[\'"]?\s*:\s*({[\s\S]*?})(?:,\s*[\'"]?\w+[\'"]?:|,\s*}|$)', props_obj)
    ...
    def _parse_props_array(self, props_array: str, component_info: VueComponentInfo) -> None:
        """
        ...
        prop_names = [name.strip().strip("'\"") for name in content.split(',') if name.strip()]
        
        for prop_name in prop_names:
            component_info.props[prop_name] = PropInfo(name=prop_name)

    def _parse_emits_interface(self, emits_interface: str, component_info: VueComponentInfo) -> None:
        """
        ...
        for emit_name, payload in emit_entries:
            emit_info = EmitInfo(
                name=emit_name,
                payload_type=payload.strip() if payload else None
            )
            component_info.emits[emit_name] = emit_info

    def _parse_emits_array(self, emits_array: str, component_info: VueComponentInfo) -> None:
        """
        ...
        emit_names = [name.strip().strip("'\"") for name in content.split(',') if name.strip()]
        
        for emit_name in emit_names:
            component_info.emits[emit_name] = EmitInfo(name=emit_name)

    def _parse_emits_object(self, emits_obj: str, component_info: VueComponentInfo) -> None:
        """
    ...
    def _parse_data_object(self, data_obj: str, component_info: VueComponentInfo) -> None:
        """
        Parse data object.
    ...
    def _parse_computed_object(self, computed_obj: str, component_info: VueComponentInfo) -> None:
        """
        ...
        computed_getters = re.findall(r'[\'"]?(\w+)[\'"]?\s*\(\s*\)\s*{([\s\S]*?)return([^;]+);', computed_obj)
        ...
        for name, getter in computed_get_set:
            setter_exists = f"set(" in computed_obj[computed_obj.find(name):computed_obj.find(name) + 200]
            computed_info = ComputedInfo(
                name=name,
                getter=getter.strip(),
                setter=setter_exists
            )
            component_info.computed[name] = computed_info

    def _parse_methods_object(self, methods_obj: str, component_info: VueComponentInfo) -> None:
        """
    ...
    def _parse_watch_object(self, watch_obj: str, component_info: VueComponentInfo) -> None:
        """
        Parse watch object.
    ...
    def _parse_components_object(self, components_obj: str, component_info: VueComponentInfo) -> None:
        """
    ...
    def _extract_imports(self, script_content: str) -> List[str]:
        ...
        # Extract ES6 imports
        import_matches = re.findall(r'import\s+([\s\S]*?)\s+from\s+[\'"]([^\'"]+)[\'"]', script_content)
...
class TypeScriptParser:
    ...
    def _extract_imports(self, content: str) -> List[str]:
        ...
        type_import_matches = re.findall(r'import\s+type\s+([\s\S]*?)\s+from\s+[\'"]([^\'"]+)[\'"];', content)
    ...
    def _extract_exports(self, content: str) -> List[str]:
        ...
        export_matches = re.findall(r'export\s+(?:const|let|var|function|class|interface|type|enum)\s+(\w+)', content)
        for name in export_matches:
            exports.append(name)
        
        # Extract default exports
        default_export_match = re.search(r'export\s+default\s+(?:const|let|var|function|class|interface|type|enum)?\s*(\w+)', content)
    ...
    def _extract_types(self, content: str) -> Dict[str, TypeInfo]:
        ...
        # Extract interfaces
        interface_matches = re.findall(r'(?:export\s+)?interface\s+(\w+)(?:\s+extends\s+([\s\S]*?))?\s*{([\s\S]*?)}', content)
        ...
        type_alias_matches = re.findall(r'(?:export\s+)?type\s+(\w+)(?:<[^>]*>)?\s*=\s*([^;]+);', content)
        for name, definition in type_alias_matches:
            types[name] = TypeInfo(
                name=name,
                kind="type",
                source_code=definition.strip()
            )
    ...
    def _extract_classes(self, content: str) -> Dict[str, Dict[str, Any]]:
        ...
        for name, extends, implements, body in class_matches:
            ...
            # Extract properties
            property_matches = re.findall(r'(private|protected|public|readonly)?\s*(\w+)\s*:\s*([^;]+);', body)
    ...
    def _extract_variables(self, content: str) -> Tuple[Dict[str, str], Dict[str, str]]:
        ...
        const_matches = re.findall(r'(?:export\s+)?const\s+(\w+)(?:\s*:\s*([^=]+))?\s*=\s*([^;]+);', content)
        ...
        var_matches = re.findall(r'(?:export\s+)?(?:let|var)\s+(\w+)(?:\s*:\s*([^=]+))?\s*=\s*([^;]+);', content)
...
class FrontendStructureMapper:
    ...
    def export_mermaid(self, output_path: Optional[Path] = None) -> Optional[str]:
        ...
        for component_name, component_info in components.items():
            ...
            for prop_name, prop_info in component_info.props.items():
                ...
                mermaid_lines.append(f"  {component_name} : +{prop_name}{required} : {prop_type}")
            
            # Add component emits as methods
            for emit_name, emit_info in component_info.emits.items():
                payload = f"({emit_info.payload_type})" if emit_info.payload_type else "()"
            ...
            for computed_name, computed_info in component_info.computed.items():
                ...
                mermaid_lines.append(f"  {component_name} : +{computed_name} : {return_type}")
            ...
            for method_name, method_info in component_info.methods.items():
                ...
                mermaid_lines.append(f"  {component_name} : +{method_name}({args}) : {return_type}")
        ...
        for component_name, component_info in components.items():
            for imported_component in component_info.components.values():
                if imported_component in components:
                    component_relationships.add(f"  {imported_component} <-- {component_name} : imports")
        ...
        for file_name, file_info in ts_files.items():
            for type_name, type_info in file_info.types.items():
                ...
                for prop_name, prop_type in type_info.properties.items():
                    mermaid_lines.append(f"  {qualified_name} : +{prop_name} : {prop_type}")
                ...
                for extends_type in type_info.extends:
                    if "." not in extends_type:
                        ...
                        if extends_type in file_info.types:
                            ...
                            mermaid_lines.append(f"  {extends_qualified} <|-- {qualified_name} : extends")
                    else:
                        mermaid_lines.append(f"  {extends_type} <|-- {qualified_name} : extends")
        ...
        if ts_files:
            for file_name, file_info in sorted(ts_files.items()):
                if file_info.types:
                    ...
                    for type_name, type_info in file_info.types.items():
                        extends = f" extends {', '.join(type_info.extends)}" if type_info.extends else ""
    ...
    def _directory_info_to_dict(self, dir_info: DirectoryInfo) -> Dict[str, Any]:
        ...
        for component_name, component_info in dir_info.vue_components.items():
            dir_dict["vue_components"][component_name] = self._component_info_to_dict(component_info)
        
        # Convert TypeScript files
        for file_name, file_info in dir_info.typescript_files.items():
            dir_dict["typescript_files"][file_name] = self._typescript_file_info_to_dict(file_info)
        
        # Convert subdirectories
        for subdir_name, subdir_info in dir_info.subdirectories.items():
            dir_dict["subdirectories"][subdir_name] = self._directory_info_to_dict(subdir_info)
        
        return dir_dict

    def _component_info_to_dict(self, component_info: VueComponentInfo) -> Dict[str, Any]:
        """
    ...
    def _typescript_file_info_to_dict(self, file_info: TypeScriptFileInfo) -> Dict[str, Any]:
        """
    ...
    def _generate_directory_tree(self) -> List[str]:
        ...
        def build_tree(path: Path, prefix: str = "", is_last: bool = True) -> List[str]:
            ...
            for i, item in enumerate(items):
                ...
                tree_lines.append(f"{item_prefix}{item.name}{os.sep if item.is_dir() else ''}")
                
                if item.is_dir():
                    tree_lines.extend(build_tree(Path(item.path), next_prefix, is_last_item))
    ...
    def _collect_all_components(self, dir_info: DirectoryInfo) -> Dict[str, VueComponentInfo]:
        """
    ...
    def _collect_all_typescript_files(self, dir_info: DirectoryInfo) -> Dict[str, TypeScriptFileInfo]:
        """
    ...
    def _component_to_markdown(self, component_info: VueComponentInfo) -> List[str]:
        ...
        if component_info.props:
            ...
            for prop_name, prop_info in sorted(component_info.props.items()):
                ...
                lines.append(f"| {prop_name} | {prop_type} | {required} | {default_value} |")
        ...
        if component_info.component_type == "Options API" and component_info.data:
            ...
            if component_info.reactive_state:
                ...
                for state_name, state_value in sorted(component_info.reactive_state.items()):
        ...
        if component_info.methods:
            ...
            for method_name, method_info in sorted(component_info.methods.items()):
                is_async = "async " if method_info.is_async else ""
                args = ", ".join(method_info.args)
                return_type = f" → {method_info.return_type}" if method_info.return_type else ""
        ...
        if component_info.components:
            sections.append(("Components", [f"`{k}` ({v})" for k, v in component_info.components.items()]))
        
        if component_info.provide:
            sections.append(("Provided Keys", [f"`{key}`" for key in component_info.provide]))
        
        if component_info.inject:
            sections.append(("Injected Keys", [f"`{key}`" for key in component_info.inject]))
        
        if component_info.directives:
            sections.append(("Directives", [f"`{directive}`" for directive in component_info.directives]))
        
        if component_info.mixins:
            sections.append(("Mixins", [f"`{mixin}`" for mixin in component_info.mixins]))
    ...
    def _typescript_types_to_markdown(self, file_info: TypeScriptFileInfo) -> List[str]:
        ...
        for type_name, type_info in sorted(file_info.types.items()):
            ...
            extends_str = f" extends {', '.join(type_info.extends)}" if type_info.extends else ""
    ...
    def _typescript_functions_to_markdown(self, file_info: TypeScriptFileInfo) -> List[str]:
        """
