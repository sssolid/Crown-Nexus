...
<template>
  <!-- Enhanced version of your NotificationSystem that uses position layouts -->
  <div class="notification-system">
    ...
    >
      ...
      >
        <!-- Use your existing Vuetify setup -->
        <v-snackbar
          v-model="snackbarVisibility[notification.id]"
          :color="notification.type"
          :timeout="-1" <!-- We handle timeout manually -->
          :location="getLocationForPosition(position)"
          :min-width="280"
          :max-width="560"
...
// Helper to get Vuetify location from our position enum
function getLocationForPosition(position: NotificationPosition): string {
  switch (position) {
...
const getNotificationsByPosition = (position: NotificationPosition) => {
  return notifications.value.filter(n => {
    // Handle legacy notifications with just 'top' or 'bottom'
    if (position === NotificationPosition.TOP && n.position === 'top') {
      return true
    }
    if (position === NotificationPosition.BOTTOM && n.position === 'bottom') {
      return true
    }
    // Handle new, more specific positions
...
watch(notifications, (newNotifications) => {
  // Add new notifications to visibility tracking
