...
<template>
  <div>
    <v-container fluid>
      ...
      <v-row class="mb-6">
        <v-col cols="12" md="8">
          <h1 class="text-h3 font-weight-bold">Media Library</h1>
          <p class="text-subtitle-1">Browse and manage all media assets</p>
        </v-col>
      ...
      <v-row>
        ...
        <v-col cols="12" md="9">
          ...
          <v-card class="mb-6">
                    ...
                    <v-col cols="12" md="4">
                      <v-select
                        ...
                        :items="[
                          ...
                          { title: 'Medium (1-5MB)', value: 'medium' },
                          { title: 'Large (> 5MB)', value: 'large' }
                        ]"
                        item-title="title"
                        item-value="value"
                        variant="outlined"
            ...
            <v-card-actions>
              <div class="d-flex align-center">
                <v-chip v-if="selected.length > 0" class="mr-2">
                ...
                <v-menu v-if="selected.length > 0">
                  ...
                  <v-list>
                    <v-list-item
                      @click="batchAssociateProduct"
                      :disabled="!canAssociateWithProduct"
                    >
                      <template v-slot:prepend>
                        <v-icon icon="mdi-link"></v-icon>
                      </template>
                      <v-list-item-title>Associate with Product</v-list-item-title>
                    </v-list-item>
                    <v-list-item
          ...
          <!-- Media Grid View -->
          <v-card v-else-if="viewMode === 'grid' && filteredMedia.length > 0">
            <v-row class="pa-2">
              <v-col
                ...
                >
                  ...
                  >
                    <v-img
                      ...
                      @dblclick="openMediaPreview(item)"
                    ></v-img>
                    <div
                      v-else
                      class="d-flex align-center justify-center non-image-thumbnail"
                      style="height: 150px;"
                      @dblclick="openMediaPreview(item)"
                    >
                      <v-icon
                  ...
                  <v-card-title class="px-3 py-2 d-flex justify-space-between text-subtitle-2">
                    ...
                    <v-menu location="bottom">
                      ...
                      <v-list>
                        <v-list-item @click="openMediaPreview(item)">
                          <template v-slot:prepend>
                            <v-icon icon="mdi-eye"></v-icon>
                          </template>
                          <v-list-item-title>Preview</v-list-item-title>
                        </v-list-item>
                        <v-list-item @click="editMedia(item)">
                          <template v-slot:prepend>
                            <v-icon icon="mdi-pencil"></v-icon>
                          </template>
                          <v-list-item-title>Edit</v-list-item-title>
                        </v-list-item>
                        <v-list-item @click="downloadMedia(item)">
                          <template v-slot:prepend>
                        ...
                        <v-divider></v-divider>
                        <v-list-item
                          @click="associateWithProduct(item)"
                          :disabled="!canAssociateWithProduct"
                        >
                          <template v-slot:prepend>
                            <v-icon icon="mdi-link"></v-icon>
          ...
          <!-- Media List View -->
          <v-card v-else-if="viewMode === 'list' && filteredMedia.length > 0">
            <v-data-table
              v-model="selected"
              :headers="headers"
              :items="filteredMedia"
          ...
          <v-card v-else-if="!loading">
            <v-card-text class="text-center py-8">
              <v-icon icon="mdi-image-off" size="x-large" class="mb-4"></v-icon>
              <h3 class="text-h6 mb-2">No Media Found</h3>
              <p class="mb-4">{{ search || Object.keys(filters).some(k => filters[k]) ? 'No media matches your search criteria.' : 'The media library is empty.' }}</p>
              <v-btn
                color="primary"
                prepend-icon="mdi-upload"
                @click="showUploadDialog = true"
          ...
          <v-card class="mb-6">
            ...
            <v-list density="compact">
              <v-list-item>
                ...
                <v-list-item-subtitle>{{ totalMediaCount }}</v-list-item-subtitle>
              </v-list-item>
              ...
              <v-list-item>
                ...
                <v-list-item-subtitle>{{ imageCount }}</v-list-item-subtitle>
              </v-list-item>
              ...
              <v-list-item>
                ...
                <v-list-item-subtitle>{{ documentCount }}</v-list-item-subtitle>
              </v-list-item>
              ...
              <v-list-item>
                ...
                <v-list-item-subtitle>{{ formatFileSize(totalSize) }}</v-list-item-subtitle>
              </v-list-item>
            </v-list>
          </v-card>
                ...
                <template v-slot:prepend>
                  <v-avatar size="40">
                    <v-img
                      ...
                      cover
                    ></v-img>
                    <v-icon v-else :icon="getFileTypeIcon(item)"></v-icon>
                  </v-avatar>
                </template>
        ...
        <v-card>
          ...
          <v-card-text class="pa-4">
            ...
            <v-select
              ...
              label="Associate with Product (Optional)"
              variant="outlined"
              :items="productOptions"
              item-title="name"
              item-value="id"
      ...
      <v-dialog v-model="showPreviewDialog" max-width="800">
        <v-card>
          ...
          <v-card-text class="pa-0">
            ...
            ></v-img>
            <div
              v-else-if="selectedMedia && !isImage(selectedMedia)"
          ...
          <v-card-text class="pa-4">
            <v-list density="compact">
              <v-list-item>
                ...
                <v-list-item-subtitle>{{ selectedMedia ? getFileTypeLabel(selectedMedia) : '' }}</v-list-item-subtitle>
              </v-list-item>
              ...
              <v-list-item>
                ...
                <v-list-item-subtitle>{{ selectedMedia ? formatDateTime(selectedMedia.created_at) : '' }}</v-list-item-subtitle>
              </v-list-item>

              <v-list-item v-if="selectedMedia?.product">
                <v-list-item-title>Associated Product</v-list-item-title>
                <v-list-item-subtitle>
                  <v-chip
                    size="small"
                    color="primary"
                    variant="tonal"
                    :to="{ name: 'ProductDetail', params: { id: selectedMedia.product.id } }"
                  >
              ...
              <v-list-item v-if="selectedMedia?.description">
                <v-list-item-title>Description</v-list-item-title>
                <v-list-item-subtitle>{{ selectedMedia.description }}</v-list-item-subtitle>
              </v-list-item>
            </v-list>
          </v-card-text>
          <v-divider></v-divider>
          <v-card-actions class="pa-4">
            ...
            <v-btn
              ...
              @click="confirmDeleteFromPreview"
            >
              Delete
            </v-btn>
          </v-card-actions>
      ...
      <v-dialog v-model="showDeleteDialog" max-width="500">
        <v-card>
          ...
          <v-card-text class="pa-4 pt-6">
            ...
            <v-alert
              v-if="mediaToDelete?.product"
              type="warning"
              variant="tonal"
              class="mt-4"
            >
              This media is associated with product "{{ mediaToDelete.product.name }}".
              Deleting it may affect product displays.
            </v-alert>
          </v-card-text>
          <v-card-actions class="pa-4">
            <v-spacer></v-spacer>
      ...
      <v-dialog v-model="showBatchDeleteDialog" max-width="500">
        <v-card>
          ...
          <v-card-text class="pa-4 pt-6">
            <p>
              Are you sure you want to delete <strong>{{ selected.length }}</strong> media files?
            </p>
            <p class="text-medium-emphasis mt-2">
              This action cannot be undone. The files will be permanently removed from the server.
            </p>
            <v-alert
              v-if="hasSelectedWithProducts"
              type="warning"
              variant="tonal"
              class="mt-4"
            >
              Some of these files are associated with products.
              Deleting them may affect product displays.
            </v-alert>
          </v-card-text>
          <v-card-actions class="pa-4">
            <v-spacer></v-spacer>
      ...
      <!-- Associate with Product Dialog -->
      <v-dialog v-model="showAssociateDialog" max-width="600">
        <v-card>
          <v-card-title class="text-h5 pa-4">
          ...
          <v-card-text class="pa-4">
            <p class="mb-4">
              Select a product to associate with
              {{ mediaToAssociate ? 'this media file' : `${selected.length} selected files` }}.
            </p>

            <v-select
              v-model="associateForm.product_id"
              label="Product"
              variant="outlined"
              :items="productOptions"
              item-title="name"
          ...
          <v-card-actions class="pa-4">
            ...
            <v-btn
              ...
              :disabled="!associateForm.product_id"
            >
              Associate
            </v-btn>
          </v-card-actions>
...
export default defineComponent({
  ...
  setup() {
    ...
    const filteredMedia = ref<Media[]>([]);
    const selected = ref<Media[]>([]);
    const recentUploads = ref<Media[]>([]);
    ...
    const mediaToAssociate = ref<Media | null>(null);
    ...
    const sortOptions = ref([
      ...
      { title: 'Size (Largest)', value: 'size_desc' },
      { title: 'Size (Smallest)', value: 'size_asc' }
    ]);
    ...
    const canAssociateWithProduct = computed(() => {
      return authStore.isAdmin;
    });

    // Check if any selected media has product associations
    const hasSelectedWithProducts = computed(() => {
      return selected.value.some(media => media.product);
    });
    ...
    const imageCount = computed(() => {
      return allMedia.value.filter(item => isImage(item)).length;
    });
    ...
    // Toggle selection of a media item (for grid view)
    ...
    // Fetch media data
    const fetchMedia = async (applyClientFilters: boolean = true) => {
      loading.value = true;
      ...
      try {
        ...
        // Convert date range to filter format if needed
        if (dateRange.value && dateRange.value.length === 2) {
          apiFilters.date_from = dateRange.value[0];
          apiFilters.date_to = dateRange.value[1];
        }

        // Get media from API
        const response = await mediaService.getMediaList(apiFilters);

        // Update state
        allMedia.value = response.items;
        ...
        notificationService.error('Failed to load media');
        filteredMedia.value = [];
      } finally {
        loading.value = false;
      }
    ...
    const applyClientSideFilters = () => {
      ...
      // Apply search if not already done on server
      if (search.value && !filters.value.search) {
    ...
    // Apply filters
    const applyFilters = () => {
      if (Object.keys(filters.value).some(key => !!filters.value[key]) || search.value) {
        // If we have significant filtering, refetch from server
        fetchMedia(true);
      } else {
        // Otherwise just apply client-side filtering to existing data
        applyClientSideFilters();
      }
    };
    ...
    const fetchRecentUploads = async () => {
      try {
        ...
        // For now, we'll just use the most recent items from all media
        const sorted = [...allMedia.value].sort(
          (a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
        );

        recentUploads.value = sorted.slice(0, 5);
      } catch (error) {
    ...
    // Download media
    const downloadMedia = (media: Media | null) => {
      if (!media) return;
    ...
    // Bulk download selected media
    const bulkDownload = () => {
      if (selected.value.length === 0) return;

      // For a real app, we'd want to create a zip file on the server
      // For this example, we'll just download them one by one
      selected.value.forEach(media => {
        downloadMedia(media);
      });

      notificationService.success(`Downloading ${selected.value.length} files`);
    };
    ...
    const editMedia = (media: Media) => {
      ...
      editForm.value = {
        ...
        product_id: media.product?.id || null
      };
    ...
    const saveMediaEdit = async () => {
      ...
      try {
        ...
        const index = allMedia.value.findIndex(m => m.id === selectedMedia.value?.id);
        if (index !== -1) {
          allMedia.value[index] = updatedMedia;
        }

        // Update filtered media
        const filteredIndex = filteredMedia.value.findIndex(m => m.id === selectedMedia.value?.id);
        if (filteredIndex !== -1) {
          filteredMedia.value[filteredIndex] = updatedMedia;
        }
    ...
    const confirmDeleteMedia = (media: Media) => {
    ...
    const deleteMedia = async () => {
      ...
      try {
        ...
        // Remove from all media lists
        allMedia.value = allMedia.value.filter(m => m.id !== mediaToDelete.value?.id);
        filteredMedia.value = filteredMedia.value.filter(m => m.id !== mediaToDelete.value?.id);
        selected.value = selected.value.filter(m => m.id !== mediaToDelete.value?.id);
        recentUploads.value = recentUploads.value.filter(m => m.id !== mediaToDelete.value?.id);

        showDeleteDialog.value = false;

        // Close preview if this was the selected media
    ...
    const batchDeleteMedia = async () => {
      ...
      try {
        ...
        const deletePromises = selected.value.map(media =>
        ...
        // Remove from all media lists
        const deletedIds = selected.value.map(m => m.id);
        allMedia.value = allMedia.value.filter(m => !deletedIds.includes(m.id));
        filteredMedia.value = filteredMedia.value.filter(m => !deletedIds.includes(m.id));
        recentUploads.value = recentUploads.value.filter(m => !deletedIds.includes(m.id));
        ...
        notificationService.success(`${deletedIds.length} media files deleted successfully`);
      } catch (error) {
        console.error('Error batch deleting media:', error);
        notificationService.error('Failed to delete selected media');
      } finally {
        batchDeleteLoading.value = false;
      }
    };

    // Associate with product (single media)
    const associateWithProduct = (media: Media) => {
      mediaToAssociate.value = media;
      associateForm.value.product_id = media.product?.id || null;
      showAssociateDialog.value = true;
    };

    // Batch associate with product
    const batchAssociateProduct = () => {
      if (selected.value.length === 0) return;

      mediaToAssociate.value = null;
      associateForm.value.product_id = null;
      showAssociateDialog.value = true;
    };

    // Confirm associate
    const confirmAssociate = async () => {
      if (!associateForm.value.product_id) return;

      associateLoading.value = true;

      try {
        if (mediaToAssociate.value) {
          // Associate single media
          const updatedMedia = await mediaService.associateMediaWithProduct(
            mediaToAssociate.value.id,
            associateForm.value.product_id
          );

          // Update in lists
          const index = allMedia.value.findIndex(m => m.id === mediaToAssociate.value?.id);
          if (index !== -1) {
            allMedia.value[index] = updatedMedia;
          }

          const filteredIndex = filteredMedia.value.findIndex(m => m.id === mediaToAssociate.value?.id);
          if (filteredIndex !== -1) {
            filteredMedia.value[filteredIndex] = updatedMedia;
          }

          notificationService.success('Media associated with product successfully');
        } else {
          // Batch associate
          const associatePromises = selected.value.map(media =>
            mediaService.associateMediaWithProduct(media.id, associateForm.value.product_id as string)
          );

          const updatedMedia = await Promise.all(associatePromises);

          // Update in lists
          updatedMedia.forEach(media => {
            const index = allMedia.value.findIndex(m => m.id === media.id);
            if (index !== -1) {
              allMedia.value[index] = media;
            }

            const filteredIndex = filteredMedia.value.findIndex(m => m.id === media.id);
            if (filteredIndex !== -1) {
              filteredMedia.value[filteredIndex] = media;
            }
          });

          notificationService.success(`${updatedMedia.length} media files associated with product successfully`);
        }
        ...
        notificationService.error('Failed to associate media with product');
      } finally {
    ...
    const cancelUpload = () => {
      ...
      uploadForm.value.product_id = null;
    ...
    onMounted(async () => {
      // Fetch product options first, so they're available for other operations
