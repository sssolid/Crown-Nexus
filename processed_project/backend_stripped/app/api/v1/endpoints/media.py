...
async def upload_file(background_tasks: BackgroundTasks, db: Annotated[AsyncSession, Depends(get_db)], current_user: Annotated[User, Depends(get_current_active_user)], file: UploadFile=File(...), media_type: MediaType=Form(MediaType.IMAGE), visibility: MediaVisibility=Form(MediaVisibility.PRIVATE), metadata: str=Form('{}'), product_id: Optional[str]=Form(None)) -> Any:
    try:
        ...
        if 'media' in locals() and hasattr(media, 'id'):
            await db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f'Error uploading file: {str(e)}') from e
...
async def get_media_file(media_id: str, db: Annotated[AsyncSession, Depends(get_db)], current_user: Optional[User]=Depends(get_optional_user)) -> Any:
    stmt = select(Media).where(Media.id == media_id)
    result = await db.execute(stmt)
    media = result.scalar_one_or_none()
    if not media:
...
async def get_media_thumbnail(media_id: str, db: Annotated[AsyncSession, Depends(get_db)], current_user: Optional[User]=Depends(get_optional_user)) -> Any:
    stmt = select(Media).where(Media.id == media_id)
    result = await db.execute(stmt)
    media = result.scalar_one_or_none()
    if not media:
