...
from app.core.exceptions import AuthenticationException, BusinessException, DatabaseException, ErrorCode, ResourceNotFoundException, ValidationException
...
class ChatService:
    ...
    async def create_room(self, name: Optional[str], room_type: ChatRoomType, creator_id: str, company_id: Optional[str]=None, members: Optional[List[Dict[str, Any]]]=None) -> ChatRoom:
        logger.info('Creating chat room', room_type=room_type, creator_id=creator_id, company_id=company_id)
        ...
        try:
            ...
            logger.info('Chat room created', room_id=str(room.id), room_type=room_type, creator_id=creator_id)
            return room
            ...
            await self.db.rollback()
            logger.error('Database error creating chat room', error=str(e), room_type=room_type, creator_id=creator_id)
            raise DatabaseException(message=f'Failed to create chat room due to database error: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
        except Exception as e:
            await self.db.rollback()
            logger.error('Unexpected error creating chat room', error=str(e), room_type=room_type, creator_id=creator_id)
            raise BusinessException(message=f'Failed to create chat room: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def get_room(self, room_id: str) -> Optional[ChatRoom]:
        ...
        try:
            ...
            raise DatabaseException(message=f'Failed to get chat room: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def get_room_with_members(self, room_id: str) -> Optional[ChatRoom]:
        ...
        try:
            ...
            logger.error('Database error getting chat room with members', error=str(e), room_id=room_id)
            raise DatabaseException(message=f'Failed to get chat room with members: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def get_user_rooms(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error('Database error getting user rooms', error=str(e), user_id=user_id)
            raise DatabaseException(message=f'Failed to get user chat rooms: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def get_room_info(self, room_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not room:
                ...
                raise ResourceNotFoundException(message=f'Chat room with ID {room_id} not found')
            members = []
            ...
            logger.error('Database error getting room info', error=str(e), room_id=room_id)
            raise DatabaseException(message=f'Failed to get chat room info: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def check_room_access(self, user_id: str, room_id: str) -> bool:
        ...
        try:
            ...
            logger.error('Database error checking room access', error=str(e), user_id=user_id, room_id=room_id)
            raise DatabaseException(message=f'Failed to check room access: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def create_message(self, room_id: str, sender_id: str, content: str, message_type: str='text', metadata: Optional[Dict[str, Any]]=None) -> ChatMessage:
        logger.info('Creating chat message', room_id=room_id, sender_id=sender_id, message_type=message_type)
        try:
        ...
        try:
            ...
            logger.info('Chat message created', message_id=str(message.id), room_id=room_id, sender_id=sender_id)
            return message
        except ValidationException:
            await self.db.rollback()
            raise
            ...
            await self.db.rollback()
            logger.error('Database error creating chat message', error=str(e), room_id=room_id, sender_id=sender_id)
            raise DatabaseException(message=f'Failed to create chat message: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
        except Exception as e:
            await self.db.rollback()
            logger.error('Unexpected error creating chat message', error=str(e), room_id=room_id, sender_id=sender_id)
            raise BusinessException(message=f'Failed to create chat message: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def edit_message(self, message_id: str, content: str) -> Tuple[bool, Optional[ChatMessage]]:
        ...
        try:
            ...
            await self.db.rollback()
            logger.error('Database error editing chat message', error=str(e), message_id=message_id)
            raise DatabaseException(message=f'Failed to edit chat message: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def delete_message(self, message_id: str) -> bool:
        logger.info('Deleting chat message', message_id=message_id)
        try:
            query = update(ChatMessage).where(ChatMessage.id == uuid.UUID(message_id)).values(is_deleted=True, deleted_at=datetime.datetime.now(datetime.UTC))
            result = await self.db.execute(query)
            await self.db.commit()
            ...
            await self.db.rollback()
            logger.error('Database error deleting chat message', error=str(e), message_id=message_id)
            raise DatabaseException(message=f'Failed to delete chat message: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def check_message_permission(self, message_id: str, user_id: str, require_admin: bool=False) -> bool:
        logger.debug('Checking message permission', message_id=message_id, user_id=user_id, require_admin=require_admin)
        try:
            ...
            logger.error('Database error checking message permission', error=str(e), message_id=message_id, user_id=user_id)
            raise DatabaseException(message=f'Failed to check message permissions: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def mark_as_read(self, user_id: str, room_id: str, last_read_id: str) -> bool:
        logger.debug('Marking messages as read', user_id=user_id, room_id=room_id, last_read_id=last_read_id)
        try:
            message_query = select(ChatMessage).where(and_(ChatMessage.id == uuid.UUID(last_read_id), ChatMessage.room_id == uuid.UUID(room_id)))
            message_result = await self.db.execute(message_query)
            message = message_result.scalar_one_or_none()
            if not message:
                logger.warning('Message not found for marking as read', last_read_id=last_read_id, room_id=room_id)
                return False
            ...
            success = result.rowcount > 0
            if success:
                logger.debug('Messages marked as read', user_id=user_id, room_id=room_id, last_read_id=last_read_id)
            else:
                logger.warning('Failed to mark messages as read - member not found', user_id=user_id, room_id=room_id)
            return success
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error('Database error marking messages as read', error=str(e), user_id=user_id, room_id=room_id)
            raise DatabaseException(message=f'Failed to mark messages as read: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def get_unread_count(self, room_id: str, user_id: str, last_read_at: Optional[datetime]=None) -> int:
        logger.debug('Getting unread message count', user_id=user_id, room_id=room_id)
        try:
            if not last_read_at:
            ...
            logger.error('Database error getting unread count', error=str(e), user_id=user_id, room_id=room_id)
            raise DatabaseException(message=f'Failed to get unread message count: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def get_message_history(self, room_id: str, before_id: Optional[str]=None, limit: int=50) -> List[Dict[str, Any]]:
        logger.debug('Getting message history', room_id=room_id, before_id=before_id, limit=limit)
        try:
            if before_id:
            ...
            logger.error('Database error getting message history', error=str(e), room_id=room_id)
            raise DatabaseException(message=f'Failed to get message history: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def add_reaction(self, message_id: str, user_id: str, reaction: str) -> bool:
        logger.debug('Adding message reaction', message_id=message_id, user_id=user_id, reaction=reaction)
        try:
            ...
            if existing:
                logger.debug('Reaction already exists', message_id=message_id, user_id=user_id, reaction=reaction)
                return True
            ...
            try:
                await self.db.commit()
                logger.info('Message reaction added', message_id=message_id, user_id=user_id, reaction=reaction)
                return True
            except Exception as e:
                await self.db.rollback()
                logger.error('Error adding reaction', error=str(e), message_id=message_id, user_id=user_id)
                return False
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error('Database error adding message reaction', error=str(e), message_id=message_id, user_id=user_id)
            raise DatabaseException(message=f'Failed to add message reaction: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def remove_reaction(self, message_id: str, user_id: str, reaction: str) -> bool:
        logger.debug('Removing message reaction', message_id=message_id, user_id=user_id, reaction=reaction)
        try:
            query = select(MessageReaction).where(and_(MessageReaction.message_id == uuid.UUID(message_id), MessageReaction.user_id == uuid.UUID(user_id), MessageReaction.reaction == reaction))
            result = await self.db.execute(query)
            reaction_obj = result.scalar_one_or_none()
            if not reaction_obj:
                logger.warning('Reaction not found for removal', message_id=message_id, user_id=user_id, reaction=reaction)
                return False
            await self.db.delete(reaction_obj)
            try:
                await self.db.commit()
                logger.info('Message reaction removed', message_id=message_id, user_id=user_id, reaction=reaction)
                return True
            except Exception as e:
                await self.db.rollback()
                logger.error('Error removing reaction', error=str(e), message_id=message_id, user_id=user_id)
                return False
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error('Database error removing message reaction', error=str(e), message_id=message_id, user_id=user_id)
            raise DatabaseException(message=f'Failed to remove message reaction: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def add_member(self, room_id: str, user_id: str, role: ChatMemberRole=ChatMemberRole.MEMBER) -> bool:
        logger.info('Adding chat room member', room_id=room_id, user_id=user_id, role=role)
        try:
            ...
            if existing:
                if not existing.is_active:
                    ...
                    existing.role = role
                    await self.db.commit()
                    logger.info('Reactivated existing chat room member', room_id=room_id, user_id=user_id)
                return True
            member = ChatMember(room_id=uuid.UUID(room_id), user_id=uuid.UUID(user_id), role=role, is_active=True)
            self.db.add(member)
            try:
                await self.db.commit()
                logger.info('Chat room member added', room_id=room_id, user_id=user_id, role=role)
                return True
            except Exception as e:
                await self.db.rollback()
                logger.error('Error adding member', error=str(e), room_id=room_id, user_id=user_id)
                return False
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error('Database error adding chat room member', error=str(e), room_id=room_id, user_id=user_id)
            raise DatabaseException(message=f'Failed to add chat room member: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def remove_member(self, room_id: str, user_id: str) -> bool:
        logger.info('Removing chat room member', room_id=room_id, user_id=user_id)
        try:
            query = select(ChatMember).where(and_(ChatMember.room_id == uuid.UUID(room_id), ChatMember.user_id == uuid.UUID(user_id), ChatMember.is_active == True))
            result = await self.db.execute(query)
            member = result.scalar_one_or_none()
            if not member:
                logger.warning('Member not found for removal', room_id=room_id, user_id=user_id)
                return False
            member.is_active = False
            try:
                await self.db.commit()
                logger.info('Chat room member removed', room_id=room_id, user_id=user_id)
                return True
            except Exception as e:
                await self.db.rollback()
                logger.error('Error removing member', error=str(e), room_id=room_id, user_id=user_id)
                return False
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error('Database error removing chat room member', error=str(e), room_id=room_id, user_id=user_id)
            raise DatabaseException(message=f'Failed to remove chat room member: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def update_member_role(self, room_id: str, user_id: str, role: ChatMemberRole) -> bool:
        logger.info('Updating chat room member role', room_id=room_id, user_id=user_id, role=role)
        try:
            query = select(ChatMember).where(and_(ChatMember.room_id == uuid.UUID(room_id), ChatMember.user_id == uuid.UUID(user_id), ChatMember.is_active == True))
            result = await self.db.execute(query)
            member = result.scalar_one_or_none()
            if not member:
                logger.warning('Member not found for role update', room_id=room_id, user_id=user_id)
                return False
            member.role = role
            try:
                await self.db.commit()
                logger.info('Chat room member role updated', room_id=room_id, user_id=user_id, role=role)
                return True
            except Exception as e:
                await self.db.rollback()
                logger.error('Error updating member role', error=str(e), room_id=room_id, user_id=user_id)
                return False
        except SQLAlchemyError as e:
            await self.db.rollback()
            logger.error('Database error updating member role', error=str(e), room_id=room_id, user_id=user_id)
            raise DatabaseException(message=f'Failed to update member role: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def find_direct_chat(self, user_id1: str, user_id2: str) -> Optional[str]:
        logger.debug('Finding direct chat', user_id1=user_id1, user_id2=user_id2)
        try:
            ...
            for room in rooms:
                ...
                member = member_result.scalar_one_or_none()
                if member:
                    logger.debug('Found direct chat', room_id=str(room.id), user_id1=user_id1, user_id2=user_id2)
                    return str(room.id)
            logger.debug('No direct chat found', user_id1=user_id1, user_id2=user_id2)
            return None
        except SQLAlchemyError as e:
            logger.error('Database error finding direct chat', error=str(e), user_id1=user_id1, user_id2=user_id2)
            raise DatabaseException(message=f'Failed to find direct chat: {str(e)}', code=ErrorCode.DATABASE_ERROR, original_exception=e) from e
    async def create_direct_chat(self, user_id1: str, user_id2: str) -> str:
        logger.info('Creating direct chat', user_id1=user_id1, user_id2=user_id2)
        existing_room_id = await self.find_direct_chat(user_id1, user_id2)
        if existing_room_id:
            logger.debug('Using existing direct chat', room_id=existing_room_id, user_id1=user_id1, user_id2=user_id2)
        ...
        logger.info('Created new direct chat', room_id=str(room.id), user_id1=user_id1, user_id2=user_id2)
        return str(room.id)
    @classmethod
