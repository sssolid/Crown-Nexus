...
class AS400Settings(BaseSettings):
    ...
    def validate_interval(cls, v: Union[int, str]) -> int:
        ...
        if v < 300:
            logger.warning(f'AS400_SYNC_INTERVAL too small ({v}s), setting to 300s minimum')
            return 300
        return v
