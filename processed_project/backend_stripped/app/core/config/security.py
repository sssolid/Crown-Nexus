...
class SecuritySettings(BaseSettings):
    ...
    RATE_LIMIT_REQUESTS_PER_MINUTE: int = Field(default=600, description='Default requests allowed per minute')
    ...
    CONTENT_SECURITY_POLICY: str = Field(default="default-src 'self'; img-src 'self' data: https://fastapi.tiangolo.com; script-src 'self' https://cdn.jsdelivr.net 'unsafe-inline'; style-src 'self' https://cdn.jsdelivr.net 'unsafe-inline'; connect-src 'self';", description='Content Security Policy header value')
    ...
    model_config = SettingsConfigDict(env_file='.env', env_file_encoding='utf-8', case_sensitive=True, extra='ignore', json_schema_extra={'ALLOWED_HOSTS': {'env_mode': 'str'}, 'TRUSTED_PROXIES': {'env_mode': 'str'}, 'BACKEND_CORS_ORIGINS': {'env_mode': 'str'}})
