...
def _initialize_default_metrics() -> None:
    create_counter(MetricName.HTTP_REQUESTS_TOTAL, 'Total number of HTTP requests', [MetricTag.METHOD, MetricTag.ENDPOINT, MetricTag.STATUS_CODE])
    create_histogram(MetricName.HTTP_REQUEST_DURATION_SECONDS, 'HTTP request duration in seconds', [MetricTag.METHOD, MetricTag.ENDPOINT])
    create_gauge(MetricName.HTTP_IN_PROGRESS, 'Number of HTTP requests in progress', [MetricTag.METHOD, MetricTag.ENDPOINT])
    create_counter(MetricName.HTTP_ERRORS_TOTAL, 'Total number of HTTP errors', [MetricTag.METHOD, MetricTag.ENDPOINT, MetricTag.ERROR_CODE])
    create_counter(MetricName.DB_QUERIES_TOTAL, 'Total number of database queries', [MetricTag.OPERATION, MetricTag.ENTITY])
    create_histogram(MetricName.DB_QUERY_DURATION_SECONDS, 'Database query duration in seconds', [MetricTag.OPERATION, MetricTag.ENTITY])
    create_counter(MetricName.DB_ERRORS_TOTAL, 'Total number of database errors', [MetricTag.OPERATION, MetricTag.ENTITY, MetricTag.ERROR_TYPE])
    create_counter(MetricName.SERVICE_CALLS_TOTAL, 'Total number of service calls', [MetricTag.COMPONENT, MetricTag.ACTION])
    create_histogram(MetricName.SERVICE_CALL_DURATION_SECONDS, 'Service call duration in seconds', [MetricTag.COMPONENT, MetricTag.ACTION])
    create_counter(MetricName.SERVICE_ERRORS_TOTAL, 'Total number of service errors', [MetricTag.COMPONENT, MetricTag.ACTION, MetricTag.ERROR_TYPE])
    create_counter(MetricName.CACHE_HIT_TOTAL, 'Total number of cache hits', [MetricTag.CACHE_BACKEND, MetricTag.COMPONENT])
    create_counter(MetricName.CACHE_MISS_TOTAL, 'Total number of cache misses', [MetricTag.CACHE_BACKEND, MetricTag.COMPONENT])
    create_histogram(MetricName.CACHE_OPERATION_DURATION_SECONDS, 'Cache operation duration in seconds', [MetricTag.CACHE_BACKEND, MetricTag.OPERATION])
    if _config.enable_process_metrics:
        create_gauge(MetricName.PROCESS_RESIDENT_MEMORY_BYTES, 'Resident memory size in bytes', [])
        create_gauge(MetricName.PROCESS_VIRTUAL_MEMORY_BYTES, 'Virtual memory size in bytes', [])
        create_counter(MetricName.PROCESS_CPU_SECONDS_TOTAL, 'Total user and system CPU time spent in seconds', [])
