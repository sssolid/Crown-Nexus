    ...
    try:
        ...
        try:
            ...
            if isinstance(e, ValidationException):
                logger.warning(f'Validation error in offset pagination for {model_class.__name__}', exc_info=e, extra={'params': params.model_dump()})
                raise
            logger.error(f'Offset pagination failed: {str(e)}', exc_info=True, extra={'model': model_class.__name__, 'params': params.model_dump()})
            raise InvalidPaginationParamsException(message=f'Pagination failed: {str(e)}', params=params.model_dump(), original_exception=e) from e
    except Exception as e:
        ...
        if metrics_service and HAS_METRICS:
            ...
            try:
                ...
                metrics_service.observe_histogram('pagination_duration_seconds', duration, {'type': 'offset', 'model': model_name, 'error': str(error or '')})
                metrics_service.increment_counter('pagination_operations_total', 1, {'type': 'offset', 'model': model_name, 'error': str(error or '')})
            except Exception as metrics_err:
                logger.warning(f'Failed to record pagination metrics: {str(metrics_err)}', exc_info=metrics_err)
async def paginate_with_cursor(db: AsyncSession, model_class: Type[DeclarativeMeta], query: Select, params: CursorPaginationParams, transform_func: Optional[Callable[[Any], Any]]=None, response_model: Optional[Type[Any]]=None) -> PaginationResult[Any]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f'Invalid cursor for {model_class.__name__}', exc_info=e, extra={'cursor': params.cursor})
            raise InvalidCursorException(message=f'Invalid cursor format: {str(e)}', cursor=params.cursor, original_exception=e) from e
        except Exception as e:
            if isinstance(e, ValidationException) or isinstance(e, InvalidCursorException):
                raise
        ...
        if metrics_service and HAS_METRICS:
            ...
            try:
                model_name = model_class.__name__.lower()
                metrics_service.observe_histogram('pagination_duration_seconds', duration, {'type': 'cursor', 'model': model_name, 'error': str(error or '')})
                metrics_service.increment_counter('pagination_operations_total', 1, {'type': 'cursor', 'model': model_name, 'error': str(error or '')})
            except Exception as metrics_err:
                logger.warning(f'Failed to record pagination metrics: {str(metrics_err)}', exc_info=metrics_err)
