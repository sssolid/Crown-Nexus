    ...
    async def paginate_with_cursor(self, query: Select, params: CursorPaginationParams, transform_func: Optional[Callable[[T], R]]=None) -> PaginationResult[R]:
        ...
        if params.cursor:
            try:
                ...
                raise ValidationException(message='Invalid cursor format', details=[{'loc': ['cursor'], 'msg': 'Invalid cursor format', 'type': 'value_error.cursor_format'}])
    ...
    def _apply_sorting(self, query: Select, sort_fields: List[SortField]) -> Select:
        for sort_field in sort_fields:
            ...
            if not hasattr(self.model_class, field_name):
                raise ValidationException(message=f'Invalid sort field: {field_name}', details=[{'loc': ['sort', 'field'], 'msg': f'Invalid sort field: {field_name}', 'type': 'value_error.sort_field'}])
    ...
    def _apply_cursor_filter(self, query: Select, sort_fields: List[SortField], cursor_values: Dict[str, Any]) -> Select:
        ...
        for i, sort_field in enumerate(sort_fields):
            ...
            if field_name not in cursor_values:
                continue
            if not hasattr(self.model_class, field_name):
                raise ValidationException(message=f'Invalid cursor field: {field_name}', details=[{'loc': ['cursor', 'field'], 'msg': f'Invalid cursor field: {field_name}', 'type': 'value_error.cursor_field'}])
            column = getattr(self.model_class, field_name)
