...
class PermissionChecker:
    ...
    def ensure_object_permission(user: 'User', obj: Any, permission: Permission, owner_field: str='created_by_id') -> None:
        if not PermissionChecker.check_object_permission(user, obj, permission, owner_field):
            ...
            logger.warning(f"Object permission denied: {getattr(user, 'email', 'Unknown')} tried to {action} {resource}", user_id=str(getattr(user, 'id', 'Unknown')), user_role=getattr(user, 'role', 'Unknown'), permission=str(permission), object_id=getattr(obj, 'id', None), object_type=obj.__class__.__name__)
            raise PermissionDeniedException(message=f"You don't have permission to {action} this {resource}", action=action, resource_type=resource, permission=str(permission))
