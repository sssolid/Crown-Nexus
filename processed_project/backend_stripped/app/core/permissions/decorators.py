    ...
    def decorator(func: T) -> T:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                await permission_service.ensure_permission(current_user, permission, resource_type=str(permission).split(':')[0])
            except Exception:
                if not PermissionChecker.has_permission(current_user, permission):
                    ...
                    logger.warning(f"Permission denied: {getattr(current_user, 'email', 'Unknown')} tried to {action} {resource}", user_id=str(getattr(current_user, 'id', 'Unknown')), user_role=getattr(current_user, 'role', 'Unknown'), permission=str(permission))
                    raise PermissionDeniedException(message=f"You don't have permission to {action} {resource}", action=action, resource_type=resource, permission=str(permission))
            try:
                metrics_service = get_service('metrics_service')
                duration = time.monotonic() - start_time
                metrics_service.observe_histogram('permission_decorator_duration_seconds', duration, {'permission': str(permission)})
            except Exception:
                pass
...
def require_permissions(permissions: List[Permission], require_all: bool=True) -> Callable[[T], T]:
    def decorator(func: T) -> T:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                permission_service = get_service('permission_service')
                has_permissions = await permission_service.check_permissions(current_user, permissions, require_all=require_all)
                if not has_permissions:
                    permission_str = ' and '.join((str(p) for p in permissions)) if require_all else ' or '.join((str(p) for p in permissions))
                    logger.warning(f"Permission denied: {getattr(current_user, 'email', 'Unknown')} missing required permissions: {permission_str}", user_id=str(getattr(current_user, 'id', 'Unknown')), user_role=getattr(current_user, 'role', 'Unknown'), permissions=[str(p) for p in permissions])
            ...
            try:
                metrics_service = get_service('metrics_service')
                duration = time.monotonic() - start_time
                metrics_service.observe_histogram('multiple_permissions_decorator_duration_seconds', duration, {'permissions_count': str(len(permissions)), 'require_all': str(require_all)})
            except Exception:
                pass
            return await func(*args, **kwargs)
        return cast(T, wrapper)
