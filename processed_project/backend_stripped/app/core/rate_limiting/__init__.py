...
async def initialize() -> None:
    ...
    try:
        ...
        metrics_service.create_counter('rate_limit_exceeded_total', 'Total number of requests that exceeded rate limits', ['path', 'client_host'])
        metrics_service.create_counter('rate_limiting_requests_total', 'Total number of requests processed by rate limiting', ['limited'])
        metrics_service.create_histogram('rate_limiting_middleware_duration_seconds', 'Time spent in rate limiting middleware', ['limited'])
        metrics_service.create_histogram('rate_limiting_check_duration_seconds', 'Time spent checking rate limits', ['storage', 'error'])
        metrics_service.create_counter('rate_limiting_checks_total', 'Total number of rate limit checks performed', ['storage', 'error'])
...
__all__ = ['RateLimitRule', 'RateLimitStrategy', 'RateLimiter', 'initialize', 'shutdown', 'check_rate_limit', 'get_ttl', 'RateLimitingException', 'RateLimitExceededException', 'RateLimitingServiceException', 'RateLimitingConfigurationException', 'RateLimitingService', 'get_rate_limiting_service']