...
def encrypt_data(data: Union[str, bytes, dict]) -> str:
    ...
    try:
        ...
        raise SecurityException(message='Failed to encrypt data', code=ErrorCode.SECURITY_ERROR, details={'error': str(e)}) from e
def decrypt_data(encrypted_data: str) -> Union[str, dict]:
    ...
    try:
        ...
        raise SecurityException(message='Failed to decrypt data', code=ErrorCode.SECURITY_ERROR, details={'error': 'Invalid encrypted data format'}) from e
def generate_secure_token(length: int=32) -> str:
    ...
    try:
        ...
        raise SecurityException(message='Failed to generate secure token', code=ErrorCode.SECURITY_ERROR, details={'error': str(e)}) from e
