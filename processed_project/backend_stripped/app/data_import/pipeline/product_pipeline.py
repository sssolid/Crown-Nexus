...
class ProductPipeline:
    def __init__(self, connector: Union[Connector, FileMakerConnector, FileConnector], processor: ProductProcessor, importer: ProductImporter, dry_run: bool=False) -> None:
    ...
    async def run(self, query: str, limit: Optional[int]=None, **params: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            if not raw_data:
                ...
                return {'success': True, 'message': 'No data extracted from source', 'records_extracted': 0, 'records_processed': 0, 'records_validated': 0, 'records_imported': 0, 'extract_time': extract_time, 'process_time': 0, 'validate_time': 0, 'import_time': 0, 'total_time': time.time() - start_time}
            ...
            if not processed_data:
                ...
                return {'success': False, 'message': 'No records processed', 'records_extracted': len(raw_data), 'records_processed': 0, 'records_validated': 0, 'records_imported': 0, 'extract_time': extract_time, 'process_time': process_time, 'validate_time': 0, 'import_time': 0, 'total_time': time.time() - start_time}
            ...
            if not validated_data:
                ...
                await self.connector.close()
                return {'success': False, 'message': 'No records validated', 'records_extracted': len(raw_data), 'records_processed': len(processed_data), 'records_validated': 0, 'records_imported': 0, 'extract_time': extract_time, 'process_time': process_time, 'validate_time': validate_time, 'import_time': 0, 'total_time': time.time() - start_time}
            logger.info(f'Validated {len(validated_data)} records in {validate_time:.2f} seconds')
            if self.dry_run:
                logger.info('Skipping import (dry run)')
                import_result = {'success': True, 'created': 0, 'updated': 0, 'errors': 0, 'total': len(validated_data), 'message': 'Dry run, no data imported'}
                import_time = 0
            else:
            ...
            return {'success': import_result.get('success', False), 'message': import_result.get('message', 'Import completed'), 'records_extracted': len(raw_data), 'records_processed': len(processed_data), 'records_validated': len(validated_data), 'records_imported': import_result.get('created', 0) + import_result.get('updated', 0), 'records_created': import_result.get('created', 0), 'records_updated': import_result.get('updated', 0), 'records_with_errors': import_result.get('errors', 0), 'error_details': import_result.get('error_details', []), 'extract_time': extract_time, 'process_time': process_time, 'validate_time': validate_time, 'import_time': import_time, 'total_time': total_time, 'dry_run': self.dry_run, 'processed_data': processed_data if self.dry_run else None}
            ...
            try:
                ...
                logger.error(f'Error closing connector: {str(close_error)}')
            total_time = time.time() - start_time
            return {'success': False, 'message': f'Pipeline failed: {str(e)}', 'records_extracted': 0, 'records_processed': 0, 'records_validated': 0, 'records_imported': 0, 'total_time': total_time, 'error': str(e)}