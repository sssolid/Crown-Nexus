...
async def transaction(db: AsyncSession) -> AsyncGenerator[AsyncSession, None]:
    ...
    try:
        ...
        raise TransactionException(message=f'Database transaction failed: {str(e)}', code=ErrorCode.TRANSACTION_FAILED, original_exception=e) from e
    except Exception as e:
        logger.error(f'Unexpected error in transaction: {str(e)}', exc_info=True)
        raise TransactionException(message=f'Unexpected error in transaction: {str(e)}', code=ErrorCode.TRANSACTION_FAILED, original_exception=e) from e
...
async def create_object(db: AsyncSession, model: Type[T], obj_in: Dict[str, Any]) -> T:
    try:
        ...
        if 'unique constraint' in error_text or 'unique violation' in error_text:
            raise DataIntegrityException(message=f'Failed to create {model.__name__}: A record with these attributes already exists', code=ErrorCode.DATA_INTEGRITY_ERROR, original_exception=e) from e
...
async def update_object(db: AsyncSession, model: Type[T], id_value: Any, obj_in: Dict[str, Any], user_id: Optional[Any]=None) -> Optional[T]:
    try:
        ...
        error_text = str(e).lower()
        if 'unique constraint' in error_text or 'unique violation' in error_text:
            raise DataIntegrityException(message=f'Cannot update {model.__name__}: A record with these attributes already exists', code=ErrorCode.DATA_INTEGRITY_ERROR, original_exception=e) from e
...
async def delete_object(db: AsyncSession, model: Type[T], id_value: Any, user_id: Optional[Any]=None, hard_delete: bool=False) -> bool:
    try:
        ...
        if 'foreign key constraint' in error_text:
            raise DataIntegrityException(message=f'Cannot delete {model.__name__}: It is referenced by other records', code=ErrorCode.DATA_INTEGRITY_ERROR, original_exception=e) from e
...
async def bulk_create(db: AsyncSession, model: Type[T], objects: List[Dict[str, Any]]) -> List[T]:
    ...
    try:
        ...
        error_text = str(e).lower()
        if 'unique constraint' in error_text or 'unique violation' in error_text:
            raise DataIntegrityException(message=f'Bulk create failed: One or more {model.__name__} instances violate uniqueness constraints', code=ErrorCode.DATA_INTEGRITY_ERROR, original_exception=e) from e
...
async def bulk_update(db: AsyncSession, model: Type[T], id_field: str, objects: List[Dict[str, Any]]) -> int:
    ...
    try:
        ...
        error_text = str(e).lower()
        if 'unique constraint' in error_text or 'unique violation' in error_text:
            raise DataIntegrityException(message=f'Bulk update failed: One or more {model.__name__} updates violate uniqueness constraints', code=ErrorCode.DATA_INTEGRITY_ERROR, original_exception=e) from e
