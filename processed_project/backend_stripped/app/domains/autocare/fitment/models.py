...
class FitmentMapping(Base):
    ...
    created_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('user.user.id'), nullable=True)
    updated_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('user.user.id'), nullable=True)
    product: Mapped['Product'] = relationship('Product', primaryjoin='FitmentMapping.product_id == foreign(Product.id)')
    created_by: Mapped['User'] = relationship('User', primaryjoin='FitmentMapping.created_by_id == foreign(User.id)')
    updated_by: Mapped['User'] = relationship('User', primaryjoin='FitmentMapping.updated_by_id == foreign(User.id)')
...
class FitmentMappingHistory(Base):
    ...
    __table_args__ = {'schema': 'product'}
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    mapping_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('product.autocare_fitment_mapping.id'), nullable=False, index=True)
