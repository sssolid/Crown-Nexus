...
class MetaUOMCode(Base):
    ...
    measurement_group_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_measurement_group.measurement_group_id'), nullable=False)
...
class PartAttributeAssignment(Base):
    ...
    pa_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_part_attribute.pa_id'), nullable=False)
    meta_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_metadata.meta_id'), nullable=False)
    ...
    uom_assignments = relationship('MetaUomCodeAssignment', back_populates='attribute_assignment')
    valid_value_assignments = relationship('ValidValueAssignment', back_populates='attribute_assignment')
...
class MetaUomCodeAssignment(Base):
    ...
    papt_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_part_attribute_assignment.papt_id'), nullable=False)
    meta_uom_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_meta_uom_code.meta_uom_id'), nullable=False)
    attribute_assignment = relationship('PartAttributeAssignment', back_populates='uom_assignments')
...
class ValidValueAssignment(Base):
    ...
    papt_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_part_attribute_assignment.papt_id'), nullable=False)
    valid_value_id: Mapped[int] = mapped_column(Integer, ForeignKey('autocare_valid_value.valid_value_id'), nullable=False)
    attribute_assignment = relationship('PartAttributeAssignment', back_populates='valid_value_assignments')
...
class PartAttributeStyle(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    style_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('autocare_style.style_id'), nullable=True)
    papt_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('autocare_part_attribute_assignment.papt_id'), nullable=True)
    style = relationship('Style', back_populates='part_attribute_styles')
    attribute_assignment = relationship('PartAttributeAssignment', back_populates='style')
    def __repr__(self) -> str:
...
class PartTypeStyle(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    style_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('autocare_style.style_id'), nullable=True)
    part_terminology_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey('autocare_parts.part_terminology_id'), nullable=True)
