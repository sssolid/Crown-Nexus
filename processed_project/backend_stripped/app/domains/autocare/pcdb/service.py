...
class PCdbService:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {'totalParts': total_parts, 'categoryCount': category_count, 'subcategoryCount': subcategory_count, 'positionCount': position_count}
    ...
    async def get_categories(self) -> List[Dict[str, Any]]:
        ...
        return [{'category_id': category.category_id, 'category_name': category.category_name} for category in categories]
    async def get_subcategories_by_category(self, category_id: int) -> List[Dict[str, Any]]:
        ...
        return [{'subcategory_id': subcategory.subcategory_id, 'subcategory_name': subcategory.subcategory_name} for subcategory in subcategories]
    async def search_parts(self, search_term: Optional[str], categories: Optional[List[int]]=None, subcategories: Optional[List[int]]=None, positions: Optional[List[int]]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
        result = await self.repository.search_parts(search_term=search_term, categories=categories, subcategories=subcategories, positions=positions, page=page, page_size=page_size)
        parts = []
        for part in result['items']:
            ...
            parts.append({'id': str(part.id), 'part_terminology_id': part.part_terminology_id, 'part_terminology_name': part.part_terminology_name, 'description': description})
    ...
    async def get_part_details(self, part_terminology_id: int) -> Dict[str, Any]:
        part = await self.repository.parts_repo.get_by_terminology_id_with_related(part_terminology_id)
        if not part:
        ...
        return {'id': str(part.id), 'part_terminology_id': part.part_terminology_id, 'part_terminology_name': part.part_terminology_name, 'description': part.description.parts_description if part.description else None, 'categories': categories, 'positions': positions, 'superseded_by': superseded_by, 'supersedes': supersedes}