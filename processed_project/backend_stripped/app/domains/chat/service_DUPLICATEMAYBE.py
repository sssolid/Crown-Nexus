...
class ChatService:
    ...
    async def create_room(self, name: Optional[str], room_type: str, creator_id: str, company_id: Optional[str]=None, members: Optional[List[Dict[str, Any]]]=None) -> ChatRoom:
        try:
            try:
                ...
                logger.warning('Invalid chat room type', room_type=room_type, valid_types=valid_types)
            ...
            if chat_room_type == ChatRoomType.DIRECT and members and (len(members) != 2):
                logger.warning('Direct chat room requires exactly 2 members', members_count=len(members) if members else 0)
                ...
                for member_data in members:
                    ...
                    if not isinstance(member_role, ChatMemberRole):
                        try:
                            ...
                            logger.warning('Invalid member role', role=member_role, valid_roles=valid_roles)
            ...
            logger.info('Chat room created', room_id=str(room.id), room_type=room_type, creator_id=creator_id, members_count=len(members) if members else 1)
            return room
            ...
            await self.db.rollback()
            logger.error('Chat room creation failed', error=str(e), room_type=room_type, creator_id=creator_id, exc_info=True)
    ...
    async def get_room(self, room_id: str) -> Optional[ChatRoom]:
        try:
            ...
            logger.error('Error getting chat room', error=str(e), room_id=room_id, exc_info=True)
            raise BusinessException(message=f'Failed to get chat room: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def get_room_with_members(self, room_id: str) -> Optional[ChatRoom]:
        try:
            ...
            room = result.scalar_one_or_none()
            if room:
                logger.debug('Chat room with members retrieved', room_id=room_id, members_count=len(room.members))
            else:
                logger.debug('Chat room not found', room_id=room_id)
            return room
        except Exception as e:
            logger.error('Error getting chat room with members', error=str(e), room_id=room_id, exc_info=True)
            raise BusinessException(message=f'Failed to get chat room with members: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def check_room_access(self, user_id: str, room_id: str) -> bool:
        try:
            ...
            logger.debug('Checked room access', user_id=user_id, room_id=room_id, has_access=has_access)
            return has_access
        except Exception as e:
            logger.error('Error checking room access', error=str(e), user_id=user_id, room_id=room_id, exc_info=True)
            raise BusinessException(message=f'Failed to check room access: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def create_message(self, room_id: str, sender_id: str, content: str, message_type: str='text', metadata: Optional[Dict[str, Any]]=None) -> ChatMessage:
        try:
            try:
                ...
                logger.warning('Invalid message type', message_type=message_type, valid_types=valid_types)
            ...
            if not has_access:
                logger.warning("User doesn't have access to room", user_id=sender_id, room_id=room_id)
                raise BusinessException(message="User doesn't have access to this chat room", code=ErrorCode.PERMISSION_DENIED, details={'user_id': sender_id, 'room_id': room_id})
            ...
            logger.info('Chat message created', message_id=str(message.id), room_id=room_id, sender_id=sender_id, message_type=message_type)
            return message
            ...
            await self.db.rollback()
            logger.error('Chat message creation failed', error=str(e), room_id=room_id, sender_id=sender_id, exc_info=True)
    ...
    async def edit_message(self, message_id: str, content: str) -> Tuple[bool, Optional[ChatMessage]]:
        try:
            ...
            await self.db.rollback()
            logger.error('Chat message edit failed', error=str(e), message_id=message_id, exc_info=True)
            raise BusinessException(message=f'Failed to edit chat message: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def delete_message(self, message_id: str) -> bool:
        try:
            ...
            await self.db.rollback()
            logger.error('Chat message deletion failed', error=str(e), message_id=message_id, exc_info=True)
            raise BusinessException(message=f'Failed to delete chat message: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    async def check_message_permission(self, message_id: str, user_id: str, require_admin: bool=False) -> bool:
        try:
            ...
            if message.sender_id == uuid.UUID(user_id):
                logger.debug('User is message sender, permission granted', user_id=user_id, message_id=message_id)
                return True
            if require_admin:
                ...
                logger.debug('Admin permission check', user_id=user_id, message_id=message_id, has_permission=has_permission)
                return has_permission
            return False
        except Exception as e:
            logger.error('Error checking message permission', error=str(e), message_id=message_id, user_id=user_id, exc_info=True)
            raise BusinessException(message=f'Failed to check message permissions: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    ...
    async def get_message_history(self, room_id: str, before_id: Optional[str]=None, limit: int=50) -> List[Dict[str, Any]]:
        try:
            ...
            logger.debug('Retrieved message history', room_id=room_id, message_count=len(messages))
            formatted_messages = []
            for message in messages:
                ...
                if content and content.startswith('enc:'):
                    try:
                        ...
                        logger.warning('Failed to decrypt message', message_id=str(message.id), error=str(e))
                formatted_messages.append({'id': str(message.id), 'room_id': str(message.room_id), 'sender_id': str(message.sender_id) if message.sender_id else None, 'sender_name': sender_name, 'message_type': message.message_type, 'content': content, 'created_at': message.created_at.isoformat(), 'updated_at': message.updated_at.isoformat(), 'is_deleted': message.is_deleted, 'reactions': reaction_data, 'metadata': message.metadata})
            return formatted_messages
        except Exception as e:
            logger.error('Error getting message history', error=str(e), room_id=room_id, exc_info=True)
            raise BusinessException(message=f'Failed to get message history: {str(e)}', code=ErrorCode.BUSINESS_LOGIC_ERROR, original_exception=e) from e
    @classmethod
