...
class Company(Base):
    ...
    headquarters_address_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('address.id'), nullable=True)
    ...
    brands: Mapped[List['Brand']] = relationship('Brand', foreign_keys='[Brand.parent_company_id]', back_populates='parent_company')
    chat_rooms: Mapped[List['ChatRoom']] = relationship('ChatRoom', back_populates='company')
