...
class ExchangeRate(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    source_currency_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('currency.id'), nullable=False, index=True)
    target_currency_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('currency.id'), nullable=False, index=True)
    ...
    __table_args__ = (UniqueConstraint('source_currency_id', 'target_currency_id', 'effective_date', name='uix_exchange_rate_source_target_date'),)
    def __repr__(self) -> str:
