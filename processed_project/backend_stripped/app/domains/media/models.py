...
class Media(Base):
    ...
    uploaded_by_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('user.user.id'), nullable=False)
    is_approved: Mapped[bool] = mapped_column(Boolean, default=False, server_default=expression.false(), nullable=False)
    approved_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('user.user.id'), nullable=True)
    approved_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
    products: Mapped[List['Product']] = relationship('Product', secondary='product.product_media', back_populates='media')
    uploaded_by: Mapped['User'] = relationship('User', foreign_keys='Media.uploaded_by_id', back_populates='uploaded_media')
    approved_by: Mapped[Optional['User']] = relationship('User', foreign_keys='Media.approved_by_id', back_populates='approved_media')
