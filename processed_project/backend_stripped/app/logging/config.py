...
def configure_structlog() -> None:
    base_processors: List[Processor] = [structlog.stdlib.add_logger_name, structlog.stdlib.add_log_level, structlog.processors.TimeStamper(fmt='iso'), structlog.processors.CallsiteParameterAdder(), structlog.processors.StackInfoRenderer(), add_request_id_processor, add_user_id_processor, add_service_info_processor]
    environment = get_environment()
    if environment == 'development':
        structlog.configure(processors=base_processors + [ConsoleRendererWithLineNumbers(colors=True)], logger_factory=structlog.stdlib.LoggerFactory(), wrapper_class=structlog.stdlib.BoundLogger, cache_logger_on_first_use=True)
    else:
        structlog.configure(processors=base_processors + [structlog.processors.format_exc_info, structlog.stdlib.ProcessorFormatter.wrap_for_formatter], logger_factory=structlog.stdlib.LoggerFactory(), wrapper_class=structlog.stdlib.BoundLogger, cache_logger_on_first_use=True)
