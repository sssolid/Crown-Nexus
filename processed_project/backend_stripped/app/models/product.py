...
class Product(Base):
    ...
    tariff_codes: Mapped[List['TariffCode']] = relationship('app.models.reference.TariffCode', secondary=product_tariff_code_association, backref='products')
    unspsc_codes: Mapped[List['UnspscCode']] = relationship('app.models.reference.UnspscCode', secondary=product_unspsc_association, backref='products')
    countries_of_origin: Mapped[List['Country']] = relationship('app.models.location.Country', secondary=product_country_origin_association, backref='products_as_origin')
    hardware_items: Mapped[List['Hardware']] = relationship('app.models.reference.Hardware', secondary=product_hardware_association, backref='products')
    colors: Mapped[List['Color']] = relationship('app.models.reference.Color', secondary=product_color_association, backref='products')
    construction_types: Mapped[List['ConstructionType']] = relationship('app.models.reference.ConstructionType', secondary=product_construction_type_association, backref='products')
    textures: Mapped[List['Texture']] = relationship('app.models.reference.Texture', secondary=product_texture_association, backref='products')
    packaging_types: Mapped[List['PackagingType']] = relationship('app.models.reference.PackagingType', secondary=product_packaging_association, backref='products')
...
class ProductSupersession(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    old_product_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('product.id'), nullable=False, index=True)
    new_product_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('product.id'), nullable=False, index=True)
    reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    changed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)
...
class Brand(Base):
    ...
    parent_company_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('company.id'), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class ProductBrandHistory(Base):
    ...
    old_brand_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('brand.id'), nullable=True)
    new_brand_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('brand.id'), nullable=False)
    changed_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('user.id'), nullable=True)
    changed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)
...
class ProductAttribute(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    product_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('product.id'), nullable=False, index=True)
    attribute_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('attribute_definition.id'), nullable=False, index=True)
    ...
    __table_args__ = (UniqueConstraint('product_id', 'attribute_id', name='uix_product_attribute'),)
    def __repr__(self) -> str:
...
class ProductPricing(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    product_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('product.id'), nullable=False, index=True)
    pricing_type_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('price_type.id'), nullable=False, index=True)
    manufacturer_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('manufacturer.id'), nullable=True, index=True)
...
class Manufacturer(Base):
    ...
    billing_address_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('address.id'), nullable=True)
    shipping_address_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey('address.id'), nullable=True)
...
class ProductStock(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    product_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('product.id'), nullable=False, index=True)
    warehouse_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey('warehouse.id'), nullable=False, index=True)
