...
class AS400SyncService:
    ...
    async def _sync_products(self, connector: AS400Connector, db: AsyncSession) -> Dict[str, Any]:
        processor_config = AS400ProcessorConfig(field_mapping={'part_number': 'spart', 'part_number_stripped': 'snschr'}, boolean_true_values=['1', 'Y', 'YES', 'TRUE', 'T'], boolean_false_values=['0', 'N', 'NO', 'FALSE', 'F'], required_fields=['part_number'], unique_key_field='part_number')
    ...
    async def _sync_measurements(self, connector: AS400Connector, db: AsyncSession) -> Dict[str, Any]:
        ...
        processor_config = AS400ProcessorConfig(field_mapping={'product_id': 'spart', 'length': 'slen1', 'width': 'swit1', 'height': 'shgt1', 'weight': 'swght'}, required_fields=['product_id'], unique_key_field='product_id')
    ...
    async def _sync_inventory(self, connector: AS400Connector, db: AsyncSession) -> Dict[str, Any]:
        ...
        processor_config = AS400ProcessorConfig(field_mapping={'product_id': 'snschr', 'warehouse_id': 'sbran', 'quantity': 'sclsk'}, required_fields=['product_id', 'warehouse_id', 'quantity'], unique_key_field='product_id')
        from app.data_import.processors.as400_processor import AS400BaseProcessor
