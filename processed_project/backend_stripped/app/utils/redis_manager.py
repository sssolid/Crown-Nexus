...
async def get_redis_pool() -> ConnectionPool:
    ...
    if _redis_pool is None:
        try:
            ...
            logger.info('Created Redis connection pool', host=settings.REDIS_HOST, port=settings.REDIS_PORT, db=settings.REDIS_DB)
        except Exception as e:
            logger.error('Failed to create Redis connection pool', error=str(e), host=settings.REDIS_HOST, port=settings.REDIS_PORT, exc_info=True)
            raise ServiceException(message='Failed to connect to Redis', code=ErrorCode.EXTERNAL_SERVICE_ERROR, details={'service': 'redis', 'host': settings.REDIS_HOST, 'port': settings.REDIS_PORT}, original_exception=e) from e
    return _redis_pool
async def get_redis_client() -> Redis:
    try:
        ...
        raise ServiceException(message='Failed to get Redis client', code=ErrorCode.EXTERNAL_SERVICE_ERROR, details={'service': 'redis'}, original_exception=e) from e
