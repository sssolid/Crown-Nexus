...
async def create_admin_user(email: str, password: str, full_name: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        async with get_db_context() as db:
            result = await db.execute(text('SELECT 1 FROM "user"."user" WHERE email = :email'), {'email': email})
            ...
            now = datetime.now()
            await db.execute(text('INSERT INTO "user"."user" (id, email, hashed_password, full_name, role, is_active, is_deleted, created_at, updated_at) VALUES (:id, :email, :hashed_password, :full_name, :role, :is_active, :is_deleted, :created_at, :updated_at)'), {'id': user_id, 'email': email, 'hashed_password': hashed_password, 'full_name': full_name, 'role': UserRole.ADMIN.value, 'is_active': True, 'is_deleted': False, 'created_at': now, 'updated_at': now})
