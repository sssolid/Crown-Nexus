...
async def user_token(normal_user) -> str:
    return create_access_token(str(normal_user.id), 'access', expires_delta=None, role=normal_user.role.value, user_data={'email': normal_user.email})
@pytest_asyncio.fixture(scope='function')
async def admin_token(admin_user) -> str:
    return create_access_token(str(admin_user.id), 'access', expires_delta=None, role=admin_user.role.value, user_data={'email': admin_user.email})
@pytest_asyncio.fixture(scope='function')
