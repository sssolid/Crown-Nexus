...
├── alembic/
│   ├── versions/
│   │   ├── 0d95e4fb1138_updated_autocared_many_to_many_tables.py
│   │   ├── 0f07e2b6660a_engineconfig_and_engineconfig2_are_.py
│   │   ├── 394f2b3d5758_add_pcdb_search_indexes.py
│   │   ├── 48a3e13baa8e_updated_autocare_many_to_many_to_.py
│   │   ├── 4ae566f159b4_qualifier_new_qualifier_id_deferred.py
│   │   ├── 660598405145_remove_year_from_vcdb_year_autocare_.py
│   │   ├── 98e9c3581d8a_updated_engineconfig_engineconfig2_and_.py
│   │   ├── 9fc1a9affe33_remove_foreign_key_constraint_on_style_.py
│   │   ├── db7baed1531c_remove_enginebase_foreign_keys.py
│   │   └── ef389cd9aa2a_initial.py
│   ├── README
│   ├── env.py
│   └── script.py.mako
├── app/
...
│   │   │   │   ├── __init__.py
│   │   │   │   ├── base_importer.py
│   │   │   │   ├── flexible_importer.py
│   │   │   │   ├── json_file_importer.py
│   │   │   │   ├── padb_importer.py
│   │   │   │   ├── pcdb_importer.py
│   │   │   │   ├── pipe_file_importer.py
│   │   │   │   ├── qdb_importer.py
│   │   │   │   └── vcdb_importer.py
│   │   │   ├── padb/
│   │   │   │   ├── __init__.py
...
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/manage_db.py`
...
Args: sql: SQL statement to execute description: Optional description of the operation
...
Returns: Dict[str, List[Tuple[str, str]]]: Dictionary mapping table names to lists of (source_column, reference_column) tuples"""
```

```python
def get_engine() -> Engine:
    """Get synchronous database engine.  Returns: Engine: SQLAlchemy engine for database operations"""
```
...
Args: message: Migration message
...
Args: revision: Revision to downgrade to, defaults to "-1"
...
Args: revision: Revision to stamp with, defaults to "head"
...
Args: revision: Revision to upgrade to, defaults to "head"
...
Args: command: Alembic command to run *args: Additional arguments to pass to the command
...
Returns: List[str]: List of table names in order of creation"""
```
...
Args: app: The FastAPI application middleware_class: The middleware class to add **options: Additional options for the middleware"""
```
...
####### Package: autocare
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/api/v1/endpoints/autocare`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/api/v1/endpoints/autocare/__init__.py`

######## Module: padb
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/api/v1/endpoints/autocare/padb.py`
...
Args: pa_id: ID of the part attribute db: Database session

Returns: Part attribute details"""
```

```python
@router.get('/measurement-groups')
async def get_measurement_groups(db) -> List[Dict[(str, Any)]]:
    """Get all measurement groups.  Args: db: Database session  Returns: List of measurement groups"""
```

```python
@router.get('/stats')
async def get_padb_stats(db) -> Dict[(str, Any)]:
    """Get PAdb statistics.  Args: db: Database session  Returns: PAdb statistics"""
```
...
Args: part_terminology_id: ID of the part db: Database session

Returns: Part attributes"""
```
...
Args: measurement_group_id: ID of the measurement group db: Database session

Returns: List of UOM codes"""
```
...
Args: papt_id: ID of the part attribute assignment db: Database session
...
async def get_version(db) -> str:
    """Get the current PADB version.  Args: db: Database session  Returns: Current PADB version as string"""
```
...
Args: search_term: Text to search for part_terminology_id: Optional filter by part terminology ID page: Page number for pagination page_size: Number of items per page db: Database session

Returns: Paginated search results"""
```

######## Module: pcdb
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/api/v1/endpoints/autocare/pcdb.py`
...
async def get_categories(db) -> List[Dict[(str, Any)]]:
    """Get all categories.  Args: db: Database session  Returns: List of categories"""
```
...
Returns: Dict containing superseded_by and supersedes lists"""
```
...
Args: category_id: ID of the category page: Page number for pagination page_size: Number of items per page db: Database session

Returns: Paginated list of parts in the category"""
```

```python
@router.get('/stats')
async def get_pcdb_stats(db) -> Dict[(str, Any)]:
    """Get PCdb statistics.  Args: db: Database session  Returns: PCdb statistics"""
```

```python
@router.get('/positions')
async def get_positions(db) -> List[Dict[(str, Any)]]:
    """Get all positions.  Args: db: Database session  Returns: List of positions"""
```
...
Args: category_id: ID of the category db: Database session
...
@router.get('/version')
async def get_version(db) -> str:
    """Get the current PCDB version.  Args: db: Database session  Returns: Current PCDB version as string"""
```
...
Args: search_term: Text to search for db: Database session
...
Args: search_term: Text to search for category_id: Optional filter by category ID subcategory_id: Optional filter by subcategory ID position_id: Optional filter by position ID page: Page number for pagination page_size: Number of items per page db: Database session

Returns: Paginated search results"""
```

######## Module: qdb
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/api/v1/endpoints/autocare/qdb.py`
...
async def get_group_numbers(db) -> List[Dict[(str, Any)]]:
    """Get all group numbers.  Args: db: Database session  Returns: List of group numbers"""
```

```python
@router.get('/languages')
async def get_languages(db) -> List[Dict[(str, Any)]]:
    """Get all languages.  Args: db: Database session  Returns: List of languages"""
```

```python
@router.get('/stats')
async def get_qdb_stats(db) -> Dict[(str, Any)]:
    """Get Qdb statistics.  Args: db: Database session  Returns: Qdb statistics"""
```
...
Args: qualifier_id: ID of the qualifier db: Database session

Returns: Qualifier details"""
```
...
Args: qualifier_id: ID of the qualifier language_id: Optional filter by language ID db: Database session
...
async def get_qualifier_types(db) -> List[Dict[(str, Any)]]:
    """Get all qualifier types.  Args: db: Database session  Returns: List of qualifier types"""
```
...
Args: group_number_id: ID of the group number page: Page number for pagination page_size: Number of items per page db: Database session

Returns: Paginated list of qualifiers in the group"""
```

```python
@router.get('/version')
async def get_version(db) -> str:
    """Get the current QDB version.  Args: db: Database session  Returns: Current QDB version as string"""
```
...
Args: search_term: Text to search for qualifier_type_id: Optional filter by qualifier type ID language_id: Optional filter by language ID page: Page number for pagination page_size: Number of items per page db: Database session

Returns: Paginated search results"""
```

######## Module: vcdb
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/api/v1/endpoints/autocare/vcdb.py`
...
Args: year_id: Year ID make_id: Make ID model_id: Model ID db: Database session

Returns: Base vehicle details or None if not found"""
```

```python
@router.get('/submodels')
async def get_all_submodels(db) -> List[Dict[(str, Any)]]:
    """Get all submodels.  Args: db: Database session  Returns: List of all submodels"""
```
...
Args: base_vehicle_id: Base vehicle ID db: Database session

Returns: Base vehicle details"""
```
...
Args: body_style_config_id: Body style configuration ID db: Database session

Returns: Body style configuration details"""
```
...
Args: brake_config_id: Brake configuration ID db: Database session

Returns: Brake configuration details"""
```

```python
@router.get('/drive-types')
async def get_drive_types(db) -> List[Dict[(str, Any)]]:
    """Get all drive types.  Args: db: Database session  Returns: List of drive types"""
```
...
Args: engine_config_id: Engine configuration ID db: Database session

Returns: Engine configuration details"""
```

```python
@router.get('/makes/{make_id}')
async def get_make_by_id(make_id, db) -> Dict[(str, Any)]:
    """Get make by ID.  Args: make_id: ID of the make db: Database session  Returns: Make details"""
```

```python
@router.get('/makes')
async def get_makes(db) -> List[Dict[(str, Any)]]:
    """Get all makes.  Args: db: Database session  Returns: List of makes"""
```
...
Args: year: Year to filter by db: Database session

Returns: List of makes for the given year"""
```

```python
@router.get('/models/{model_id}')
async def get_model_by_id(model_id, db) -> Dict[(str, Any)]:
    """Get model by ID.  Args: model_id: ID of the model db: Database session  Returns: Model details"""
```
...
Args: year: Year to filter by make_id: Make ID to filter by db: Database session

Returns: List of models for the given year and make"""
```

```python
@router.get('/regions')
async def get_regions(db) -> List[Dict[(str, Any)]]:
    """Get all regions.  Args: db: Database session  Returns: List of regions"""
```
...
Args: parent_id: Parent region ID db: Database session

Returns: List of child regions"""
```
...
Returns: List of submodels for the given base vehicle"""
```
...
Args: transmission_id: Transmission ID db: Database session

Returns: Transmission details"""
```

```python
@router.get('/stats')
async def get_vcdb_stats(db) -> Dict[(str, Any)]:
    """Get VCdb statistics.  Args: db: Database session  Returns: VCdb statistics"""
```

```python
@router.get('/vehicles/{vehicle_id}')
async def get_vehicle_by_id(vehicle_id, db) -> Dict[(str, Any)]:
    """Get vehicle by ID.  Args: vehicle_id: Vehicle ID db: Database session  Returns: Vehicle details"""
```
...
Args: vehicle_id: Vehicle ID use_config2: Whether to use the newer EngineConfig2 model (default: True) db: Database session

Returns: Vehicle configurations"""
```
...
Args: vehicle_id: Vehicle ID db: Database session
...
async def get_vehicle_types(db) -> List[Dict[(str, Any)]]:
    """Get all vehicle types.  Args: db: Database session  Returns: List of vehicle types"""
```
...
Args: group_id: Vehicle type group ID db: Database session

Returns: List of vehicle types in the specified group"""
```

```python
@router.get('/version')
async def get_version(db) -> str:
    """Get the current VCDB version.  Args: db: Database session  Returns: Current VCDB version as string"""
```

```python
@router.get('/wheel-bases')
async def get_wheel_bases(db) -> List[Dict[(str, Any)]]:
    """Get all wheel bases.  Args: db: Database session  Returns: List of wheel bases"""
```
...
async def get_years(db) -> List[Dict[(str, Any)]]:
    """Get all years.  Args: db: Database session  Returns: List of years"""
```
...
Args: year: Optional filter by year make: Optional filter by make name model: Optional filter by model name page: Page number for pagination page_size: Number of items per page db: Database session
...
Args: engine_base_id: Optional filter by engine base ID fuel_type_id: Optional filter by fuel type ID aspiration_id: Optional filter by aspiration ID page: Page number for pagination page_size: Number of items per page db: Database session
...
Returns: List of matching makes"""
```
...
Returns: List of matching submodels"""
```
...
Args: transmission_type_id: Optional filter by transmission type ID transmission_num_speeds_id: Optional filter by number of speeds ID transmission_control_type_id: Optional filter by control type ID page: Page number for pagination page_size: Number of items per page db: Database session
...
Args: year: Optional filter by year make: Optional filter by make name model: Optional filter by model name submodel: Optional filter by submodel name engine_config: Optional filter by engine configuration transmission_type: Optional filter by transmission type body_type: Optional filter by body type page: Page number for pagination page_size: Number of items per page db: Database session

Returns: Paginated search results"""
```

#### Package: cli
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/cli`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/cli/__init__.py`
...
##### Module: autocare
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/cli/autocare.py`
...
##### Module: base
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/base.py`
...
Args: package_path: Import path to the package to scan.

Returns: Dict mapping backend names to backend classes."""
```
...
@abc.abstractmethod
    def component_name(self) -> str:
        """Get the component name.  Returns: The component name."""
```
```python
    async def health_check(self) -> Dict[(str, Any)]:
        """Perform a health check.  Returns: Dict containing health status information."""
```
```python
...
Args: component: The component to register."""
```
```python
...
All core services should inherit from this class to ensure consistent behavior and lifecycle management."""
```
*Methods:*
```python
    async def __aenter__(self) -> 'CoreService':
        """Enter the context manager.  Returns: The service instance."""
```
```python
...
Args: *args: Positional arguments for the service constructor. **kwargs: Keyword arguments for the service constructor.
...
Returns: Context manager that initializes and shuts down the service."""
```
```python
@classmethod
...
Raises: RuntimeError: If the service instance doesn't exist."""
```
```python
    async def health_check(self) -> Dict[(str, Any)]:
        """Perform a health check.  Returns: Dict containing health status information."""
...
@property
    def is_initialized(self) -> bool:
        """Check if the service is initialized.  Returns: True if the service is initialized, False otherwise."""
```
```python
    def register_component(self, component) -> None:
...
@abc.abstractmethod
    def service_name(self) -> str:
        """Get the service name.  Returns: The service name."""
```
```python
    async def shutdown(self) -> None:
...
Args: settings_prefix: Prefix for settings keys. settings_obj: Settings object, defaults to global settings.
...
Returns: Service instance or None if not found."""
```
```python
@classmethod
    def get_all(cls) -> Dict[(str, CoreService)]:
        """Get all registered services.  Returns: Dictionary of service names to service instances."""
```
```python
@classmethod
...
@classmethod
    def register(cls, service_name, service) -> None:
        """Register a service.  Args: service_name: Name of the service. service: Service instance."""
```
```python
@classmethod
...
##### Package: audit
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/__init__.py`
...
Args: db: Database session for database operations.

Returns: The audit service instance."""
```

###### Module: base
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/base.py`
...
Args: event_type: Type of event being logged. level: Log level for the event. context: Context information for the event. details: Additional event details. options: Options for logging the event.

Returns: ID of the logged event."""
```
...
###### Module: exceptions
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/exceptions.py`
...
Args: backend_name: Name of the backend that failed. operation: Name of the operation that failed. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
Args: message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
Args: message: Human-readable error message. code: Error code from ErrorCode enum. details: Additional error details. status_code: HTTP status code. original_exception: Original exception if this is a wrapper."""
```
...
Args: operation: Name of the operation that failed. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```

###### Module: manager
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/manager.py`
...
```python
    def __init__(self, db) -> None:
        """Initialize the audit manager.  Args: db: Database session for database operations."""
```
```python
    def add_backend(self, backend) -> None:
        """Add an audit backend to the manager.  Args: backend: Backend to add."""
```
```python
@property
    def component_name(self) -> str:
...
Args: event_id: ID of the event to get.

Returns: Event details or None if not found."""
```
```python
...
Args: start_time: Filter by start time (ISO format). end_time: Filter by end time (ISO format). event_type: Filter by event type. level: Filter by log level. user_id: Filter by user ID. resource_id: Filter by resource ID. resource_type: Filter by resource type. limit: Maximum number of events to return. offset: Offset for pagination.

Returns: Dictionary containing total count and list of events."""
```
```python
    async def get_resource_history(self, resource_type, resource_id, limit) -> List[Dict[(str, Any)]]:
        """Get audit events for a specific resource.

Args: resource_type: Type of the resource. resource_id: ID of the resource. limit: Maximum number of events to return.
...
Args: user_id: ID of the user. start_time: Filter by start time (ISO format). end_time: Filter by end time (ISO format). limit: Maximum number of events to return.
...
Args: days_to_keep: Number of days to keep logs for.

Returns: Number of logs deleted."""
```

###### Module: models
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/models.py`
...
```python
    def __repr__(self) -> str:
        """Get string representation of the audit log.  Returns: String representation."""
```
```python
    def to_dict(self) -> Dict[(str, Any)]:
        """Convert to dictionary.  Returns: Dictionary representation."""
```

###### Module: schemas
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/schemas.py`
...
###### Module: service
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/service.py`
...
This service coordinates audit logging across different destinations and provides query capabilities for retrieving audit logs."""
```
*Methods:*
```python
    def __init__(self, db) -> None:
        """Initialize the audit service.  Args: db: Database session for database operations."""
```
```python
    async def get_event_by_id(self, event_id) -> Optional[Dict[(str, Any)]]:
        """Get an audit event by ID.
...
Args: event_type: Type of event being logged. user_id: ID of the user who performed the action. ip_address: IP address the action was performed from. resource_id: ID of the resource being acted upon. resource_type: Type of resource being acted upon. details: Additional event details. level: Log level for the event. options: Options for logging the event.
...
###### Module: utils
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/utils.py`
...
Args: data: Data to anonymize. sensitive_fields: List of sensitive field names.
...
Returns: Dictionary mapping event types to log levels."""
```

```python
def get_sensitive_fields() -> List[str]:
    """Get the default list of sensitive fields to anonymize.  Returns: List of sensitive field names."""
```

###### Package: backends
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/backends`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/backends/__init__.py`
...
Returns: List of created backend instances."""
```
...
Args: name: Name of the backend to get. **kwargs: Additional arguments for the backend.
...
Args: name: Name of the backend. backend_class: Backend class to register.

Raises: ValueError: If a backend with the given name is already registered."""
```

####### Module: database
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/backends/database.py`
...
```python
    def __init__(self, db) -> None:
        """Initialize the database audit backend.  Args: db: Database session for database operations."""
```
```python
    async def health_check(self) -> Dict[(str, Any)]:
        """Perform a health check.  Returns: Dict containing health status information."""
...
####### Module: file
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/backends/file.py`
...
```python
    def __init__(self, file_path) -> None:
        """Initialize the file audit backend.  Args: file_path: Path to the log file."""
```
```python
    async def health_check(self) -> Dict[(str, Any)]:
        """Perform a health check.  Returns: Dict containing health status information."""
...
```python
    async def initialize(self) -> None:
        """Initialize the backend.  This ensures the log directory exists."""
```
```python
    async def log_event(self, event_type, level, context, details, options) -> str:
...
####### Module: logging
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/audit/backends/logging.py`
...
This allows accessing Filemaker settings through the main settings object while keeping them modularly separated.
...
This allows accessing Security settings through the main settings object while keeping them modularly separated.

Returns: SecuritySettings: The Security settings object"""
```
```python
@model_validator(mode='after')
...
####### Module: filemaker
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/config/integrations/filemaker.py`
...
Returns: Dictionary with Filemaker connector configuration"""
```
...
###### Module: service
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/exceptions/service.py`
...
Args: service_name: Name of the service with invalid configuration. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
Args: message: Human-readable error message. code: Error code from ErrorCode enum. details: Additional error details. status_code: HTTP status code. severity: Error severity level. original_exception: Original exception if this is a wrapper."""
```
...
Args: manager_name: Name of the manager that failed. operation: Name of the operation that failed. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
Args: service_name: Name of the service that failed to initialize. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
Args: service_name: Name of the service that is not initialized. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
Args: service_name: Name of the service that failed to shut down. message: Human-readable error message. details: Additional error details. original_exception: Original exception if this is a wrapper."""
```
...
This class provides methods to check if users have specific permissions based on their roles and explicit permissions."""
```
*Methods:*
```python
@staticmethod
...
Args: user: The user to check obj: The object to check permissions for permission: The permission to check owner_field: The field name containing the owner's ID

Returns: bool: True if the user has permission for this object"""
```
```python
@staticmethod
...
Args: user: The user to check obj: The object to check permissions for permission: The permission to check owner_field: The field indicating object ownership

Raises: PermissionDeniedException: If the user doesn't have the permission"""
```
```python
@staticmethod
...
Args: user: The user to check permissions for permission: The permission to check

Returns: bool: True if the user has the permission"""
```
```python
@staticmethod
...
Args: user: The user to check permissions for permissions: List of permissions to check require_all: Whether all permissions are required (True) or any (False)

Returns: bool: True if the user has the required permissions"""
```
...
Returns: Callable: Decorator function requiring SYSTEM_ADMIN permission"""
```
...
Args: permission: The permission required to execute the function
...
Args: permissions: List of permissions to check require_all: Whether all permissions are required (True) or any (False)
...
###### Module: service
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/permissions/service.py`
...
This service manages permission checking, caching of permission results, integration with metrics, and proper error handling for permission operations."""
```
*Methods:*
```python
    def __init__(self, db) -> None:
...
Args: db: Optional database session for operations requiring database access"""
```
```python
...
Args: user: The user to check permissions for permission: The permission to check resource_id: Optional resource ID for contextual logging resource_type: Optional resource type for contextual logging
...
Args: user: The user to check permissions for permissions: List of permissions to check require_all: Whether all permissions are required (True) or any (False) resource_id: Optional resource ID for contextual logging resource_type: Optional resource type for contextual logging
...
Args: user: The user to check permission: The permission to check resource_type: The type of resource being accessed resource_id: Optional ID of the resource being accessed
...
Args: user_id: The user ID to get permissions for

Returns: Set[Permission]: Set of all permissions the user has

Raises: AuthenticationException: If the user doesn't exist"""
```
```python
    async def initialize(self) -> None:
...
```python
    async def invalidate_permissions_cache(self, user_id) -> None:
        """Invalidate cached permissions for a user.  Args: user_id: The user ID to invalidate permissions for"""
```
```python
    async def shutdown(self) -> None:
...
Args: user_id: The user ID entity_user_id: The user ID stored in the entity owner_field: The field name used for ownership

Returns: bool: True if the user is the owner"""
```
...
Args: db: Database session user_id: ID of the user to retrieve

Returns: User: The user object

Raises: AuthenticationException: If the user is not found"""
```
...
Args: user: The user to check permissions: List of permission strings to check

Returns: bool: True if the user has all of the permissions"""
```
...
Returns: bool: True if the user has any of the permissions"""
```
...
Args: token: The JWT token request: Optional request object for metrics and context

Returns: str: The user ID from the token
...
Args: token: The JWT token request: Optional request object for metrics and context db: Optional database session

Returns: dict: User data with permissions
...
Args: token: The optional JWT token request: Optional request object for metrics and context

Returns: Optional[str]: The user ID from the token, or None if no valid token"""
```
...
Args: token: The token from the OAuth2 scheme request: Optional request object for metrics and context

Returns: str: The validated token
...
###### Module: service
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/core/security/service.py`
...
This service integrates all security components (tokens, passwords, encryption, etc.) and connects them with the application's core systems like metrics, caching, error handling, and event publication."""
```
*Methods:*
```python
    def __init__(self, db) -> None:
...
Args: user_id: The user ID to include in the token role: The user's role permissions: Optional list of user permissions user_data: Optional additional user data

Returns: TokenPair: The generated access and refresh tokens"""
```
```python
    def decrypt_data(self, encrypted_data) -> Union[(str, dict)]:
        """Decrypt data using the encryption manager.

Args: encrypted_data: The encrypted data
...
Returns: bool: True if suspicious content is detected"""
```
```python
    def encrypt_data(self, data) -> str:
...
Args: user_id: The user ID to associate with the key name: A name for the API key permissions: Optional list of permissions
...
Raises: SecurityException: If token generation fails"""
```
```python
    def get_security_headers(self) -> Dict[(str, str)]:
        """Get security headers for HTTP responses.  Returns: Dict[str, str]: Dictionary of security headers"""
```
```python
    def hash_password(self, password) -> str:
        """Hash a password using the configured password manager.

Args: password: The plaintext password
...
Returns: bool: True if the IP is trusted"""
```
```python
    async def refresh_tokens(self, refresh_token, request_id) -> TokenPair:
        """Refresh tokens using a valid refresh token.

Args: refresh_token: The refresh token request_id: Optional request ID for tracking

Returns: TokenPair: New access and refresh tokens

Raises: AuthenticationException: If the refresh token is invalid"""
```
```python
    async def revoke_token(self, token, user_id, reason, request_id) -> None:
        """Revoke a token and publish an event.

Args: token: The token to revoke user_id: The user ID associated with the token reason: The reason for revocation request_id: Optional request ID for tracking
...
Args: token: The CSRF token to validate session_id: The session ID to validate against
...
Args: password: The password to validate user_id: Optional user ID for checking password history

Returns: Tuple[bool, Optional[str]]: Validity and error message"""
```
```python
    async def validate_token(self, token, request_id) -> TokenClaimsModel:
        """Validate a JWT token with metrics tracking.

Args: token: The JWT token to validate request_id: Optional request ID for tracking

Returns: TokenClaimsModel: The validated token data

Raises: AuthenticationException: If the token is invalid or expired"""
```
```python
    def verify_api_key(self, api_key, stored_hash) -> bool:
        """Verify an API key against a stored hash.

Args: api_key: The API key to verify stored_hash: The stored hash to compare against

Returns: bool: True if the API key is valid"""
```
```python
...
Args: plain_password: The plaintext password hashed_password: The hashed password user_id: Optional user ID for tracking

Returns: bool: True if the password is correct"""
```
...
Args: token_jti: The JWT ID of the token expires_at: When the token expires"""
```
...
Args: subject: The subject of the token (typically user ID) token_type: The type of token (e.g., access, refresh) expires_delta: Optional custom expiration time role: Optional user role permissions: Optional list of permissions user_data: Optional additional user data
...
Args: user_id: The user ID role: The user's role permissions: Optional list of permissions user_data: Optional additional user data
...
Args: token: The JWT token to decode

Returns: TokenClaimsModel: The decoded token data

Raises: AuthenticationException: If the token is invalid or blacklisted"""
```

```python
def generate_token_jti() -> str:
    """Generate a unique JWT ID (jti) for a token.  Returns: str: Unique token identifier"""
```
...
Returns: bool: True if the token is blacklisted"""
```
...
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/field_definitions.py`
...
def boolean_transformation(value) -> bool:
    """Transform a value to a boolean.  Args: value: Input value  Returns: Boolean value"""
```

```python
def clean_part_number(value) -> str:
    """Clean and standardize a part number.  Args: value: Part number  Returns: Cleaned part number"""
```
...
Args: entity_name: Name of the entity to query (e.g., 'product', 'product_pricing') source_type: Type of data source (e.g., 'filemaker', 'as400', 'csv') fields: Optional list of field names to include in the query

Returns: SQL query string

Raises: ValueError: If the entity or source type is not supported ValueError: If no fields are available for the query"""
```
...
def strip_whitespace(value) -> str:
    """Strip whitespace from a string value.  Args: value: Input value  Returns: Cleaned string"""
```
...
Args: model_name: Name for the generated model
...
Args: source_type: Source type identifier (e.g., 'filemaker', 'as400', 'csv')

Returns: Dictionary mapping internal field names to external field names"""
```
```python
...
Returns: Field definition or None if not found"""
```
```python
    def get_tables_for_source(self, source_type) -> Dict[(str, List[str])]:
        """Get all tables used for a given source type.

Args: source_type: Source type identifier

Returns: Dictionary mapping table names to lists of field names from that table"""
```
...
Returns: Dictionary mapping source types to external field names"""
```
```python
...
###### Module: import_all
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/commands/import_all.py`
...
Args: verbosity: The verbosity level (quiet, normal, verbose, debug)"""
```
...
This command handles importing products, pricing, stock, and other entity types from the specified source, with options for controlling logging verbosity and audit tracking."""
```
...
Args: user_id: Optional user ID string

Returns: UUID of the system user to use for audit logging"""
```

**Classes:**
```python
class LoggingVerbosity(object):
```
*Class attributes:*
```python
QUIET = 'quiet'
...
This command extracts entity data from a source using field definitions, processes and validates it, and imports it into the database."""
```
...
###### Module: product_pricing_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/importers/product_pricing_importer.py`
...
This importer handles the conversion of part numbers and pricing type names to their corresponding UUIDs before importing pricing data into the database."""
```
*Methods:*
```python
    def __init__(self, db) -> None:
...
Args: db: SQLAlchemy async session for database operations"""
```
```python
    async def import_data(self, data) -> Dict[(str, Any)]:
        """Import product pricing data.

This method converts part numbers and pricing type names to their corresponding UUIDs, then creates or updates pricing records in the database.

Args: data: List of product pricing import records
...
This pipeline handles the extract, transform, and load process for product data, supporting different data sources and product-related entity types.

Attributes: connector: Data source connector processor: Data processor importer: Data importer dry_run: Whether to skip the import phase"""
```
*Methods:*
```python
...
Args: connector: Data source connector processor: Data processor importer: Data importer dry_run: Whether to skip the import phase"""
```
```python
    async def run(self, query, limit, **params) -> Dict[(str, Any)]:
...
Args: query: Query to extract data limit: Optional limit on the number of records to extract **params: Additional parameters for the query
...
###### Module: generic_processor
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/processors/generic_processor.py`
...
Uses field definitions from the central source of truth to ensure consistent field handling across different data sources."""
```
*Methods:*
```python
...
Args: field_definitions: Field definitions for the entity model_type: Pydantic model for validation source_type: Source type identifier (e.g., 'filemaker', 'as400', 'csv')"""
```
```python
    async def process(self, data) -> List[Dict[(str, Any)]]:
...
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/processors/integrated_processor.py`
...
class IntegratedProductProcessor(GenericProcessor[ProductCreate]):
    """Integrated processor for product data that handles descriptions, marketing, and pricing in a unified manner."""
```
*Methods:*
```python
    def __init__(self, source_type) -> None:
        """Initialize the integrated product processor.

Args: source_type: Type of data source (filemaker, as400, csv)"""
```
```python
    async def process_pricing(self, data) -> List[Dict[(str, Any)]]:
        """Process pricing data for products.  Args: data: Raw pricing data  Returns: Processed pricing data"""
```
```python
    async def validate_pricing(self, data) -> List[ProductPricingImport]:
        """Validate pricing data.

Args: data: Processed pricing data

Returns: List of validated ProductPricingImport objects"""
```

###### Module: pricing_processor
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/processors/pricing_processor.py`
...
This processor handles the transformation of AS400 product pricing data, where multiple price types (Jobber and Export) exist in a single record, into separate records for each price type.

Attributes: source_type: The type of data source (as400, filemaker, csv)"""
```
*Methods:*
```python
    def __init__(self, source_type) -> None:
...
Args: source_type: The type of data source (as400, filemaker, csv)"""
```
```python
    async def process(self, data) -> List[Dict[(str, Any)]]:
        """Process pricing data from AS400, transforming each row into multiple pricing records.

For AS400 sources, each input row with part number, jobber price, and export price is transformed into two separate records - one for each price type.

Args: data: List of raw data records from the source

Returns: List of processed records ready for validation"""
```

###### Module: processor_factory
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/processors/processor_factory.py`
...
Args: entity_type: Type of entity to process source_type: Type of data source **kwargs: Additional arguments for the processor

Returns: A processor instance

Raises: ValueError: If entity type or source type is unknown"""
```
...
This processor can handle cases where the database returns field aliases instead of the original field names."""
```
*Methods:*
```python
    def __init__(self, source_type) -> None:
...
##### Package: schemas
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/schemas`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/schemas/__init__.py`
...
###### Module: pricing
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/data_import/schemas/pricing.py`
...
This schema uses string identifiers (part number, pricing type name) instead of UUIDs to make it easier to import data from external sources.

Attributes: part_number: Product part number (string identifier) pricing_type: String identifier for the pricing type (e.g., "Jobber", "Export") price: The monetary value of the product currency: Three-letter currency code (default: USD)"""
```
...
##### Module: alembic_command
*Enhanced Alembic command module that automatically fixes self-referential table issues.*
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/db/alembic_command.py`
...
This function identifies self-referential foreign key constraints in CREATE TABLE statements and modifies them to: 1. Remove the FK constraint from the initial table creation 2. Add a separate ALTER TABLE statement to add the constraint after creation 3. Add a unique constraint on the referenced column if needed"""
```
...
###### Package: commands
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/commands`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/commands/__init__.py`
...
####### Module: import_autocare
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/commands/import_autocare.py`
...
def display_import_result(db_name, result) -> None:
    """Display import result for a database.

Args: db_name: Database name result: Import result dictionary"""
```

```python
def get_database_source_dir(source_dir, database) -> Path:
    """Get the source directory for a specific database.

Args: source_dir: Base source directory database: Database type

Returns: Path: Database-specific source directory"""
```

```python
@app.command(name='import')
def import_autocare(database, source_dir, format, batch_size, dry_run, track_in_history, verbose) -> None:
    """Import AutoCare standard data into the application database.

This command imports VCdb, PCdb, PAdb, and Qdb data from various data formats into the application database.

Example: import-autocare import vcdb --source-dir=/path/to/vcdb_files --format=json"""
```

```python
async def import_database(db, database, source_dir, source_format, batch_size, dry_run, track_in_history, verbose) -> Dict:
    """Import a specific AutoCare database.

Args: db: Database session database: AutoCare database to import source_dir: Directory containing the data files source_format: Format of the source data batch_size: Number of records to process in each batch dry_run: Validate the source data but don't import it track_in_history: Track the import in the sync history verbose: Enable verbose output
...
async def run_import(database, source_dir, source_format, batch_size, dry_run, track_in_history, verbose) -> Union[(Dict, Dict[(str, Dict)])]:
    """Run the AutoCare database import process.

Args: database: AutoCare database to import source_dir: Directory containing the data files source_format: Format of the source data batch_size: Number of records to process in each batch dry_run: Validate the source data but don't import it track_in_history: Track the import in the sync history verbose: Enable verbose output

Returns: Union[Dict, Dict[str, Dict]]: Import results"""
```
...
###### Package: importers
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers`

**__init__.py:**
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/__init__.py`
...
####### Module: base_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/base_importer.py`
...
```python
    async def import_data(self) -> Dict[(str, Any)]:
        """Import data from source to database.  Returns: Dict[str, Any]: Dictionary with import results"""
```
```python
    async def validate_source(self) -> bool:
        """Validate that the source data is valid.  Returns: bool: True if valid, False otherwise"""
```

####### Module: flexible_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/flexible_importer.py`
...
Args: source_path: Path to source directory

Returns: SourceFormat: Detected source format"""
```
...
Args: source_name: Name of the source (file name, table name, etc.)

Returns: Iterator of record dictionaries"""
```
```python
@abstractmethod
    def read_version(self) -> str:
        """Read version information from the data source.  Returns: str: Version string (typically YYYY-MM-DD)"""
```
```python
@abstractmethod
...
Args: required_sources: List of required source names
...
```python
    def __init__(self, source_path, encoding):
        """Initialize FileReader.  Args: source_path: Path to source files directory encoding: File encoding"""
```
```python
    def validate(self, required_sources) -> bool:
        """Validate that the required files exist.

Args: required_sources: List of required file names

Returns: bool: True if all required files exist, False otherwise"""
```
...
Args: db: Database session source_path: Path to source files directory schema_name: Database schema name required_sources: List of required sources version_class: Class for version records source_format: Format of the source data version_date_field: Field name for version date batch_size: Batch size for imports encoding: File encoding"""
```
```python
    async def import_data(self) -> Dict[(str, Any)]:
        """Import data from source to database.  Returns: Dict[str, Any]: Dictionary with import results"""
...
Args: source_name: Name of the source table_name: Name of the table field_mapping: Mapping from source fields to table fields transformers: Optional dict of transformer functions for fields"""
```
```python
...
Args: source_name: Name of the source model_class: SQLAlchemy model class field_mapping: Mapping from source fields to model fields primary_key: Primary key field in the model transformers: Optional dict of transformer functions for fields validators: Optional dict of validator functions for fields"""
```
```python
    def set_import_order(self, order) -> None:
        """Set the order in which tables should be imported.

Args: order: List of source names in import order"""
```
```python
    async def validate_source(self) -> bool:
        """Validate that the source data is valid.  Returns: bool: True if valid, False otherwise"""
...
Args: source_name: Name of the file

Returns: Iterator of record dictionaries"""
```
```python
    def read_version(self) -> str:
        """Read version information from the version file.  Returns: str: Version string (YYYY-MM-DD)"""
```
...
Args: source_path: Path to source files directory encoding: File encoding delimiter: Field delimiter in files"""
```
```python
    def read_records(self, source_name) -> Iterator[Dict[(str, Any)]]:
        """Read records from the specified file.
...
####### Module: json_file_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/json_file_importer.py`
...
Args: db: Database session source_path: Path to source files directory schema_name: Database schema name required_files: List of required files version_class: Class for version records version_date_field: Field name for version date batch_size: Batch size for imports encoding: File encoding"""
```
```python
    async def import_data(self) -> Dict[(str, Any)]:
        """Import data from JSON files to database.  Returns: Dict[str, Any]: Dictionary with import results"""
```
```python
    def register_many_to_many_table(self, file_name, table_name, field_mapping, transformers) -> None:
        """Register a mapping for a many-to-many table.

Args: file_name: Name of the file table_name: Name of the table field_mapping: Mapping from file columns to table fields transformers: Optional dict of transformer functions for fields"""
```
```python
...
Args: file_name: Name of the file model_class: SQLAlchemy model class field_mapping: Mapping from file columns to model fields primary_key: Primary key field in the model transformers: Optional dict of transformer functions for fields validators: Optional dict of validator functions for fields"""
```
```python
    def set_import_order(self, order) -> None:
        """Set the order in which tables should be imported.  Args: order: List of file names in import order"""
```
```python
    async def validate_source(self) -> bool:
...
####### Module: padb_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/padb_importer.py`
...
class PAdbImporter(FlexibleImporter):
    """Importer for PAdb (Part Attribute Database) data from various formats."""
```
*Methods:*
```python
...
Args: db: Database session source_path: Path to source files directory source_format: Format of the source data (auto-detected if None) batch_size: Batch size for imports"""
```

####### Module: pcdb_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/pcdb_importer.py`
...
####### Module: pipe_file_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/pipe_file_importer.py`
...
**Classes:**
```python
class PipeFileImporter(Generic[T]):
    """Generic importer for pipe-separated files."""
```
*Methods:*
```python
    def __init__(self, db, source_path, schema_name, required_files, version_class, version_date_field, batch_size, encoding, delimiter) -> None:
        """Initialize PipeFileImporter.

Args: db: Database session source_path: Path to source files directory schema_name: Database schema name required_files: List of required files version_class: Class for version records version_date_field: Field name for version date batch_size: Batch size for imports encoding: File encoding delimiter: Field delimiter in files"""
```
```python
    async def import_data(self) -> Dict[(str, Any)]:
        """Import data from pipe-separated files to database.

Returns: Dict[str, Any]: Dictionary with import results"""
```
```python
    def register_many_to_many_table(self, file_name, table_name, field_mapping, transformers) -> None:
        """Register a mapping for a many-to-many table.
...
```python
    async def validate_source(self) -> bool:
        """Validate that the source data consists of valid pipe-separated files.

Returns: bool: True if valid, False otherwise"""
```

####### Module: qdb_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/qdb_importer.py`
...
####### Module: vcdb_importer
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/autocare/importers/vcdb_importer.py`
...
```python
    async def get_stats(self) -> Dict[(str, Any)]:
        """Get PAdb statistics.  Returns: Dictionary with PAdb statistics"""
```
```python
...
Args: search_term: The search term. categories: Optional list of category IDs to filter by. subcategories: Optional list of subcategory IDs to filter by. positions: Optional list of position IDs to filter by. page: The page number. page_size: The number of items per page.
...
Returns: The part with related entities if found, None otherwise."""
```
```python
...
```python
    async def get_stats(self) -> Dict[(str, Any)]:
        """Get PCdb statistics.  Returns: Dictionary with PCdb statistics"""
```
```python
...
```python
    async def get_stats(self) -> Dict[(str, Any)]:
        """Get Qdb statistics.  Returns: Dictionary with Qdb statistics"""
```
```python
    async def get_version(self) -> str:
...
This class is maintained for backward compatibility but has been replaced by EngineBase2. New code should use EngineBase2 instead.

Attributes: id: Primary key. engine_base_id: VCdb specific ID. liter: Engine displacement in liters. cc: Engine displacement in cubic centimeters. cid: Engine displacement in cubic inches. cylinders: Number of cylinders. block_type: Configuration of the engine block. eng_bore_in: Engine bore in inches. eng_bore_metric: Engine bore in millimeters. eng_stroke_in: Engine stroke in inches. eng_stroke_metric: Engine stroke in millimeters."""
```
*Class attributes:*
```python
...
```python
    def __repr__(self) -> str:
        """Return string representation of EngineBase2 instance.  Returns: String representation."""
```
...
Attributes: id: Primary key. engine_config_id: VCdb specific ID. engine_base_id: Reference to engine base. engine_block_id: Reference to engine block. engine_bore_stroke_id: Reference to engine bore stroke. engine_designation_id: Reference to engine designation. engine_vin_id: Reference to engine VIN. valves_id: Reference to valves. fuel_delivery_config_id: Reference to fuel delivery config. aspiration_id: Reference to aspiration. cylinder_head_type_id: Reference to cylinder head type. fuel_type_id: Reference to fuel type. ignition_system_type_id: Reference to ignition system type. engine_mfr_id: Reference to engine manufacturer. engine_version_id: Reference to engine version. power_output_id: Reference to power output. engine_base: Relationship to engine base. engine_block: Relationship to engine block. engine_bore_stroke: Relationship to engine bore stroke. engine_designation: Relationship to engine designation. engine_vin: Relationship to engine VIN. valves: Relationship to valves. fuel_delivery_config: Relationship to fuel delivery config. aspiration: Relationship to aspiration. cylinder_head_type: Relationship to cylinder head type. fuel_type: Relationship to fuel type. ignition_system_type: Relationship to ignition system type. engine_mfr: Relationship to engine manufacturer. engine_version: Relationship to engine version. power_output: Relationship to power output."""
```
*Class attributes:*
```python
...
```python
    def __repr__(self) -> str:
        """Return string representation of EngineConfig2 instance.  Returns: String representation."""
```
...
Attributes: id (UUID): Primary key, a universally unique identifier for the record. vehicle_to_bed_config_id (int): Unique identifier for the vehicle-to-bed configuration. vehicle_id (int): Foreign key referencing the vehicle table. bed_config_id (int): Foreign key referencing the bed configuration table. source (str): Represents the source information, optional."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_body_style_config_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. body_style_config_id (int): Foreign key linking to the 'body_style_config' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
This class maps the relationship between a vehicle and a brake configuration in the database, recording and storing the association along with optional metadata regarding the source system. It primarily serves as part of a schema in the vcdb module.

Attributes: id (uuid.UUID): The primary key uniquely identifying each entry. config_id (int): The unique identifier for this specific vehicle to brake configuration. vehicle_id (int): Foreign key referencing the associated vehicle ID. brake_config_id (int): Foreign key referencing the associated brake configuration ID. source_system (Optional[str]): An optional metadata field indicating the data source of the configuration, limited to 10 characters for storage."""
```
*Class attributes:*
```python
...
This class defines the relationship between a vehicle and a drive type, providing a mapping structure used within the vcdb schema. It supports optional source metadata for tracking the origin of the data.

Attributes: id (uuid.UUID): Primary key uniquely identifying each entry. vehicle_to_drive_type_id (int): Unique identifier for this specific vehicle-to-drive-type mapping. vehicle_id (int): Foreign key referencing the associated vehicle. drive_type_id (int): Foreign key referencing the associated drive type. source (Optional[str]): Optional metadata field indicating the data origin, limited to 10 characters."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_engine_config_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. engine_config_id (int): Foreign key linking to the 'engine_config2' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_mfr_body_code_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. mfr_body_code_id (int): Foreign key linking to the 'mfr_body_code' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_spring_type_config_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. spring_type_config_id (int): Foreign key linking to the 'spring_type_config' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_steering_config_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. steering_config_id (int): Foreign key linking to the 'steering_config' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_transmission_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. transmission_id (int): Foreign key linking to the 'transmission' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
Attributes: id (UUID): Primary key, a unique identifier for each mapping record, generated automatically. vehicle_to_wheel_base_id (int): A unique identifier for the relationship, indexed and must not be null. vehicle_id (int): Foreign key linking to the 'vehicle' table in the 'vcdb' schema, must not be null. wheel_base_id (int): Foreign key linking to the 'wheel_base' table in the 'vcdb' schema, must not be null. source (str): An optional string (up to 10 characters) to denote the source of the mapping."""
```
*Class attributes:*
```python
...
Attributes: id: Primary key. year_id: VCdb specific ID. base_vehicles: Relationship to base vehicles."""
```
*Class attributes:*
```python
__tablename__ = 'year'
...
This class provides methods for accessing and querying data related to the EngineBase2 database model, utilizing the functionality of the BaseRepository. It allows for the retrieval of data by specific identifiers and searching based on optional filter criteria, with support for pagination.

Attributes: db (AsyncSession): The asynchronous database session to be used for queries."""
```
*Methods:*
```python
    def __init__(self, db) -> None:
...
This method performs a database query to fetch an EngineBase2 object that matches the specified engine_base_id. It includes related data for engine_block and engine_bore_stroke using selectinload to optimize loading associated relationships. If no matching record is found, it returns None.

Args: engine_base_id: An integer representing the id of the engine base to retrieve.

Returns: An instance of EngineBase2 if found, otherwise None."""
```
```python
    async def search_by_criteria(self, engine_block_id, engine_bore_stroke_id, page, page_size) -> Dict[(str, Any)]:
        """Perform a search for engine records based on the given criteria and paginate the results.

This coroutine constructs a database query for `EngineBase2` objects and applies filtering criteria based on provided engine block ID and/or engine bore-stroke ID. The query supports eager loading for related `engine_block` and `engine_bore_stroke` data. Results are paginated using the specified page number and page size.

Parameters: engine_block_id : Optional[int] A unique identifier for filtering engines by their associated engine block. If not provided, this filter will not be applied. engine_bore_stroke_id : Optional[int] A unique identifier for filtering engines by their associated bore-stroke. If not provided, this filter will not be applied. page : int The page number to retrieve the results for. Defaults to 1. page_size : int The number of results per page. Defaults to 20.

Returns: Dict[str, Any] A dictionary containing the paginated set of results based on the query."""
```
...
Returns: Dictionary with vehicle and all its component collections"""
```
```python
...
class EngineConfig2(BaseModel):
    """Schema for engine configuration2 data."""
```
*Class attributes:*
```python
model_config =     model_config = ConfigDict(from_attributes=True)
...
```python
    async def get_engine_base2(self, engine_base_id) -> Dict[(str, Any)]:
        """Fetches and returns detailed engine base information by its ID.

This asynchronous method retrieves details of the engine base including its associated engine block and engine bore stroke, if available, from the repository. If no engine base is found with the given ID, a ResourceNotFoundException is raised.

Parameters: engine_base_id (int): The unique identifier of the engine base.

Returns: Dict[str, Any]: A dictionary containing the engine base information, including nested details about the engine block and engine bore stroke.

Raises: ResourceNotFoundException: If the engine base with the specified ID does not exist."""
```
```python
    async def get_engine_config2(self, engine_config_id) -> Dict[(str, Any)]:
        """Retrieve the full configuration details for a specific engine by its ID.

Parameters: engine_config_id: int The ID of the engine configuration to retrieve.

Returns: Dict[str, Any]: A dictionary containing the full details of the engine configuration.

Raises: ResourceNotFoundException: Raised if no engine configuration is found with the specified ID."""
```
```python
...
```python
    async def get_stats(self) -> Dict[(str, Any)]:
        """Get VCdb statistics.  Returns: Dictionary with VCdb statistics"""
```
```python
...
The method fetches configuration data related to various aspects of vehicle design, including engines, transmissions, drive types, body styles, brake configurations, and wheel bases. The data is categorized and structured in grouped lists for further use.

Args: vehicle_id (int): The unique identifier for the vehicle whose configurations are being retrieved.

Raises: ResourceNotFoundException: If no configurations are found for the provided vehicle ID.

Returns: Dict[str, List[Dict[str, Any]]]: A dictionary containing grouped lists of configuration data. Each key represents a configuration category such as "engines", "transmissions", "drive_types", "body_styles", "brake_configs", and "wheel_bases". The values are lists of dictionaries holding the specific details for each configuration type."""
```
```python
...
Args: vehicle_id: The vehicle ID to retrieve details for

Returns: Dictionary containing vehicle details with components
...
This function asynchronously retrieves engine base data by querying a repository using optional filtering parameters and pagination. It builds a structured response containing information about engine bases, including their ID, liter value, cubic capacity, and number of cylinders.

Parameters: engine_block_id: Optional[int] An optional integer representing the ID of an engine block to filter the search results. engine_bore_stroke_id: Optional[int] An optional integer representing the ID of an engine bore stroke to filter the search results. page: int The page number for paginated results. page_size: int The maximum number of results per page.

Returns: Dict[str, Any] A dictionary containing the paginated list of engine bases and additional metadata about the pagination."""
```
```python
...
This asynchronous method fetches engine configuration data using the provided criteria such as engine base ID, fuel type ID, and aspiration ID. Pagination options are also available to control the number of results returned per page and the specific page to retrieve.

Args: engine_base_id: An optional ID of the engine base to filter configurations. fuel_type_id: An optional ID of the fuel type to filter configurations. aspiration_id: An optional ID of the aspiration to filter configurations. page: An integer specifying the page number to retrieve. Defaults to 1. page_size: An integer specifying the number of items per page. Defaults to 20.

Returns: A dictionary containing the filtered engine configurations, total number of items, current page, requested page size, and total number of pages. Each configuration in the 'items' list includes relevant details about the engine base, fuel type, aspiration, engine block liter size, and number of cylinders."""
```
```python
...
###### Module: associations
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/domains/products/associations.py`
...
Attributes: product_id: Unique identifier of the product pricing_type_id: Reference to the pricing type price: The monetary value of the product currency: Three-letter currency code (default: USD) manufacturer_id: Optional reference to the manufacturer"""
```
...
Inherits all fields from ProductPricingBase without modifications. Used specifically in the data import process."""
```
...
Attributes: price: Updated price value if provided currency: Updated currency code if provided manufacturer_id: Updated manufacturer reference if provided"""
```
...
```python
    def __init__(self, db):
        """Initialize the product service.  Args: db: Database session"""
```
```python
...
Args: products_data: List of product creation data user_id: ID of the user performing the action sync_source: Source of synchronization (as400, filemaker, etc.)

Returns: Dictionary with counts of created, updated, and failed records"""
```
```python
...
Args: pricing_data_list: List of pricing data user_id: ID of the user performing the action sync_source: Source of synchronization (as400, filemaker, etc.)
...
Args: data: Product creation data user_id: ID of the user performing the action import_id: ID of the import operation if part of an import

Returns: The created product

Raises: DuplicatePartNumberException: If a product with the same part number already exists"""
```
```python
    async def create_supersession(self, old_product_id, new_product_id, reason, user_id) -> ProductSupersession:
        """Create a product supersession.

Args: old_product_id: ID of the product being replaced new_product_id: ID of the replacement product reason: Reason for the supersession user_id: ID of the user performing the action

Returns: The created supersession record

Raises: ProductNotFoundException: If either product doesn't exist BusinessException: If trying to supersede a product with itself"""
```
```python
    async def delete_product(self, product_id, user_id) -> None:
        """Delete a product (soft delete).

Args: product_id: ID of the product to delete user_id: ID of the user performing the action

Raises: ProductNotFoundException: If the product doesn't exist"""
```
```python
...
Args: product_id: ID of the product pricing_data: Pricing data user_id: ID of the user performing the action import_id: ID of the import operation if part of an import

Returns: The updated/created pricing record
...
Args: product_id: ID of the product to update data: Product update data user_id: ID of the user performing the action import_id: ID of the import operation if part of an import

Returns: The updated product

Raises: ProductNotFoundException: If the product doesn't exist DuplicatePartNumberException: If updating to a part number that already exists"""
```
```python
    async def update_product_status(self, product_id, status, reason, user_id) -> Tuple[(Product, ProductActivity)]:
        """Update a product's status.

Args: product_id: ID of the product to update status: New status (active/inactive) reason: Reason for the status change user_id: ID of the user performing the action

Returns: Tuple of the updated product and the activity record
...
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/middleware/cache_control.py`
...
This middleware adds appropriate cache control headers to responses based on the request path and method to control client and CDN caching."""
```
*Methods:*
```python
...
Args: app: The FastAPI application cache_rules: Dictionary mapping path prefixes to cache control directives default_rule: Default cache control directive for paths not in cache_rules cacheable_methods: HTTP methods that can be cached (default: GET, HEAD) exclude_paths: List of paths to exclude from cache control"""
```
```python
...
Returns: The response with added cache control headers"""
```

##### Module: compression
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/middleware/compression.py`
...
This middleware compresses response bodies using gzip compression to reduce network bandwidth and improve client performance."""
```
*Methods:*
```python
...
Args: app: The FastAPI application minimum_size: Minimum size in bytes for compression to be applied compression_level: Compression level (1-9, higher means more compression) exclude_paths: List of paths to exclude from compression"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Returns: The response, potentially compressed"""
```

##### Module: cors
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/middleware/cors.py`
...
This middleware extends FastAPI's built-in CORSMiddleware to provide more detailed logging and metrics for CORS requests."""
```
*Methods:*
```python
...
Args: scope: The ASGI connection scope receive: The ASGI receive function send: The ASGI send function"""
```
```python
...
Args: app: The ASGI application allow_origins: List of allowed origins or "*" for wildcard allow_methods: List of allowed HTTP methods or "*" for wildcard allow_headers: List of allowed HTTP headers or "*" for wildcard allow_credentials: Whether to allow credentials (cookies) allow_origin_regex: Regex pattern for allowed origins expose_headers: Headers to expose to the browser max_age: Max age for CORS preflight requests in seconds"""
```
...
```python
    def __init__(self, app) -> None:
        """Initialize the error handler middleware.  Args: app: The ASGI application"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
        """Process the request and handle any exceptions.

Args: request: The incoming request call_next: Function to call the next middleware

Returns: Response: The response, or an error response if an exception occurred"""
```
...
Args: app: FastAPI application ignore_paths: List of path prefixes to exclude from metrics track_paths_without_match: Whether to track paths that don't match a route"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Args: app: The ASGI application rules: Rate limit rules to apply use_redis: Whether to use Redis for rate limiting enable_headers: Whether to include rate limit headers in responses block_exceeding_requests: Whether to block requests that exceed rate limits fallback_to_memory: Whether to fallback to memory-based rate limiting if Redis fails limit_by_path: Whether to apply rate limits per path"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Returns: Response: The response, or a rate limit exceeded response"""
```
...
Args: app: The ASGI application exclude_paths: List of paths to exclude from formatting skip_binary_responses: Whether to skip formatting binary responses"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Args: app: The FastAPI application block_suspicious_requests: Whether to block suspicious requests suspicious_patterns: List of suspicious string patterns suspicious_regex_patterns: List of suspicious regex patterns exclude_paths: List of paths to exclude from security checks"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Args: app: The FastAPI application content_security_policy: Optional CSP header value permissions_policy: Optional permissions policy header value expect_ct: Optional Expect-CT header value hsts_max_age: Max age for HSTS in seconds include_subdomains: Include subdomains in HSTS preload: Include preload directive in HSTS exclude_paths: List of paths to exclude from headers"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
##### Module: timeout
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/middleware/timeout.py`
...
class TimeoutException(AppException):
    """Exception raised when a request exceeds the timeout duration."""
```
*Methods:*
```python
...
Args: message: The error message details: Additional error details status_code: The HTTP status code code: The error code"""
```
...
Args: app: The ASGI application timeout_seconds: The maximum request duration in seconds exclude_paths: Paths to exclude from timeout enforcement"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Returns: Response: The response, or a timeout error response"""
```

##### Module: tracing
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/middleware/tracing.py`
...
This middleware sets up tracing spans and propagates tracing headers to enable request tracking across services and components."""
```
*Methods:*
```python
...
Args: app: The FastAPI application service_name: Name of this service for tracing exclude_paths: List of paths to exclude from tracing"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Args: query: SQLAlchemy Select query. page: Page number. page_size: Page size.
...
Args: items: List of items pagination: Pagination information message: Response message code: HTTP status code meta: Additional metadata request_id: Request identifier

Returns: PaginatedResponse: Paginated response instance"""
```
...
Args: message: Error message code: HTTP status code data: Optional error data meta: Additional metadata request_id: Request identifier

Returns: Response: Error response instance"""
```
```python
@classmethod
...
Args: data: Response data message: Response message code: HTTP status code meta: Additional metadata pagination: Pagination information request_id: Request identifier

Returns: Response: Success response instance"""
```
...
##### Module: circuit_breaker_utils
Path: `/home/runner/work/Crown-Nexus/Crown-Nexus/backend/app/utils/circuit_breaker_utils.py`
