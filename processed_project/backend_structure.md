...
Generated on 2025-04-04 20:42:56
...
Returns: bool: True if successful or if no tables to drop, False otherwise"""
```
...
```python
def fix_postgresql_data_types(sql) -> str:
    """Fix PostgreSQL data type incompatibilities in SQL strings.

Args: sql: SQL statement that might contain incompatible data types

Returns: str: SQL statement with PostgreSQL compatible data types"""
```
...
def reset_database() -> bool:
    """Complete database reset that actually works.  Returns: bool: True if successful, False otherwise"""
```
...
product =     product = relationship("Product", foreign_keys="[product.product_id]")
created_by =     created_by = relationship("User", foreign_keys="[user.created_by_id]")
updated_by =     updated_by = relationship("User", foreign_keys="[user.updated_by_id]")
```
*Methods:*
```python
    def __repr__(self) -> str:
...
Args: app: The FastAPI application. rules: List of rate limit rules to apply. If None, a default rule will be used. use_redis: Whether to use Redis for rate limiting. enable_headers: Whether to include rate limit headers in responses. block_exceeding_requests: Whether to block requests that exceed the rate limit. fallback_to_memory: Whether to fallback to memory if Redis fails limit_by_path: Whether to apply rate limits by path in addition to rules"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Args: app: The FastAPI application exclude_paths: List of paths to exclude from formatting skip_binary_responses: Whether to skip formatting for binary responses"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Args: message: Human-readable error message details: Additional error details status_code: HTTP status code code: Application-specific error code"""
```
...
This middleware sets a maximum execution time for requests to prevent long-running requests from consuming resources."""
```
*Methods:*
```python
...
Args: app: The FastAPI application timeout_seconds: Maximum request execution time in seconds exclude_paths: List of paths to exclude from timeout enforcement"""
```
```python
    async def dispatch(self, request, call_next) -> Response:
...
Raises: TimeoutException: If the request exceeds the timeout"""
```
