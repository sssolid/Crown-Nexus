...
**Functions:**
```python
def bad_request(message, details) -> BadRequestException:
...
```python
def business_logic_error(message, details) -> BusinessLogicException:
...
```python
def configuration_error(message, details) -> ConfigurationException:
    """Create a ConfigurationException with the provided message.

Args: message: Error message details: Additional error details

Returns: ConfigurationException with the provided message"""
```

```python
def database_error(message, original_error, details) -> DatabaseException:
    """Create a DatabaseException with the provided message.

Args: message: Error message original_error: Original exception that was caught details: Additional error details
...
Args: value: Value to check resource_type: Type of resource for error message resource_id: ID of the resource for error message message: Custom error message (optional)
...
```python
def external_service_error(service_name, message, original_error, details) -> ExternalServiceException:
    """Create an ExternalServiceException for a specific service.

Args: service_name: Name of the external service message: Error message original_error: Original exception that was caught details: Additional error details

Returns: ExternalServiceException with details for the specified service"""
```

```python
def invalid_state(entity_type, entity_id, current_state, expected_state, allowed_states) -> InvalidStateException:
    """Create an InvalidStateException for a specific entity.

Args: entity_type: Type of entity (e.g., "Order", "Payment") entity_id: ID of the entity current_state: Current state of the entity expected_state: Expected state (if one specific state was expected) allowed_states: List of allowed states (if multiple states were allowed)

Returns: InvalidStateException with details for the specified entity"""
```

```python
def operation_not_allowed(operation, entity_type, entity_id, reason) -> OperationNotAllowedException:
    """Create an OperationNotAllowedException for a specific operation.

Args: operation: Operation that was attempted (e.g., "cancel", "refund") entity_type: Type of entity (e.g., "Order", "Payment") entity_id: ID of the entity (optional) reason: Reason why the operation is not allowed (optional)

Returns: OperationNotAllowedException with details for the specified operation"""
```
...
Args: action: Action attempted (e.g., "create", "update") resource_type: Type of resource (e.g., "User", "Product") permission: Permission that was required

Returns: PermissionDeniedException with a standardized message"""
```

```python
def rate_limit_exceeded(service_name, limit, reset_after) -> RateLimitException:
    """Create a RateLimitException for a specific service.

Args: service_name: Name of the service limit: Rate limit that was exceeded reset_after: Seconds until rate limit resets

Returns: RateLimitException with details for the specified service"""
```
...
Args: resource_type: Type of resource (e.g., "User", "Product") identifier: Identifier(s) that caused the conflict field: Field name that caused the conflict message: Custom error message (optional)
...
Args: resource_type: Type of resource (e.g., "User", "Product") resource_id: ID of the resource that wasn't found message: Custom error message (optional)

Returns: ResourceNotFoundException with a standardized message"""
```

```python
def service_unavailable(service_name, message) -> ServiceUnavailableException:
    """Create a ServiceUnavailableException for a specific service.

Args: service_name: Name of the service message: Custom error message (optional)

Returns: ServiceUnavailableException with details for the specified service"""
```
...
Args: field: Field name or path message: Error message error_type: Error type
