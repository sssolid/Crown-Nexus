...
async def convert_currency(conversion: ConversionRequest, db: Annotated[AsyncSession, Depends(get_db)], current_user: Annotated[User, Depends(get_current_active_user)]) -> Any:
    ...
    return {'source_currency': conversion.source_currency, 'target_currency': conversion.target_currency, 'source_amount': conversion.amount, 'converted_amount': converted_amount, 'exchange_rate': rate, 'timestamp': datetime.utcnow()}
