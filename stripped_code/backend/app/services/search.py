...
class SearchService:
    ...
    async def search_products(self, search_term: Optional[str]=None, category_id: Optional[str]=None, attributes: Optional[Dict[str, Any]]=None, is_active: Optional[bool]=None, page: int=1, page_size: int=20, use_elasticsearch: bool=True) -> Dict[str, Any]:
        if use_elasticsearch and search_term:
            es_client = await self.get_es_client()
            if es_client:
                try:
                    return await self._search_products_elasticsearch(es_client, search_term, category_id, attributes, is_active, page, page_size)
                except Exception as e:
                    print(f'Elasticsearch error: {e}')
        return await self._search_products_database(search_term, category_id, attributes, is_active, page, page_size)
    async def _search_products_database(self, search_term: Optional[str]=None, category_id: Optional[str]=None, attributes: Optional[Dict[str, Any]]=None, is_active: Optional[bool]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
    ...
    async def _search_products_elasticsearch(self, es_client: AsyncElasticsearch, search_term: str, category_id: Optional[str]=None, attributes: Optional[Dict[str, Any]]=None, is_active: Optional[bool]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
        ...
        if category_id:
            query['query']['bool']['filter'].append({'term': {'category_id': category_id}})
        if is_active is not None:
        ...
        products = []
        if product_ids:
            query = select(Product).where(Product.id.in_(product_ids)).options(selectinload(Product.category))
    ...
    async def search_fitments(self, search_term: Optional[str]=None, year: Optional[int]=None, make: Optional[str]=None, model: Optional[str]=None, engine: Optional[str]=None, transmission: Optional[str]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
        ...
        return await paginate(self.db, query, page, page_size)
    async def search_categories(self, search_term: Optional[str]=None, parent_id: Optional[str]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
        query = select(Category)
        if search_term:
            search_pattern = f'%{search_term.lower()}%'
            query = query.where(or_(func.lower(Category.name).like(search_pattern), func.lower(Category.slug).like(search_pattern), func.lower(Category.description).like(search_pattern)))
        if parent_id:
    ...
    async def global_search(self, search_term: str, entity_types: Optional[List[str]]=None, page: int=1, page_size: int=20) -> Dict[str, Any]:
        ...
        if 'categories' in entity_types:
            category_results = await self.search_categories(search_term=search_term, page=page, page_size=page_size)
