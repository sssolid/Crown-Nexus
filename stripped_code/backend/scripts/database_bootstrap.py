...
async def create_admin_user(email: str, password: str, full_name: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        async with get_db_context() as db:
            ...
            now = datetime.now()
            await db.execute(text('INSERT INTO "user" (id, email, hashed_password, full_name, role, is_active, created_at, updated_at) VALUES (:id, :email, :hashed_password, :full_name, :role, :is_active, :created_at, :updated_at)'), {'id': user_id, 'email': email, 'hashed_password': hashed_password, 'full_name': full_name, 'role': UserRole.ADMIN.value, 'is_active': True, 'created_at': now, 'updated_at': now})
