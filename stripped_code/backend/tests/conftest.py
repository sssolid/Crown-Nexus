...
async def test_category(db: AsyncSession) -> Category:
    category = Category(name='Test Category', slug='test-category', description='A test category for unit testing')
    db.add(category)
    await db.commit()
...
async def test_product(db: AsyncSession, test_category: Category) -> Product:
    product = Product(sku='TEST-001', name='Test Product', description='A test product for unit testing', part_number='TP001', category_id=test_category.id, attributes={'material': 'steel', 'weight': 1.5}, is_active=True)
    db.add(product)
    await db.commit()
